...
for net in "${NETWORKS[@]}"; do
    ...
    if [ -f "$ENV_FILE" ]; then
        ...
        KEYPAIR_PASSWORD="$(read_env_value "$ENV_FILE" "LICHEN_KEYPAIR_PASSWORD")"
    fi
    ...
    if [ -z "$KEYPAIR_PASSWORD" ]; then
        KEYPAIR_PASSWORD=$(openssl rand -base64 32 2>/dev/null | tr -d '/+=' | head -c 32)
        echo "   🔑 Generated new LICHEN_KEYPAIR_PASSWORD (32 chars random)"
    else
        echo "   🔑 Preserved existing LICHEN_KEYPAIR_PASSWORD from previous env file"
    fi
    ...
    if [ -n "$CURRENT_HOST_IPV4" ]; then
        EXTERNAL_ADDR="$CURRENT_HOST_IPV4:$P2P_PORT"
    fi
    cat > "$ENV_FILE" <<EOF
...
LICHEN_EXTERNAL_ADDR=$EXTERNAL_ADDR
# Keypair encryption password. All services sharing encrypted keypair files
# (validator, genesis, custody, signer) must use the same password.
# Auto-generated by setup.sh if not previously set.
LICHEN_KEYPAIR_PASSWORD=$KEYPAIR_PASSWORD
...
echo ""
echo "   0. LICHEN_KEYPAIR_PASSWORD has been auto-generated in /etc/lichen/env-<net>."
echo "      The same password is in /etc/lichen/custody-env[-mainnet]."
echo "      To view or export your validator keypair later:"
echo "        LICHEN_KEYPAIR_PASSWORD=\$(grep LICHEN_KEYPAIR_PASSWORD /etc/lichen/env-<net> | cut -d= -f2-)"
echo "        export LICHEN_KEYPAIR_PASSWORD"
echo "        lichen identity export --keypair /var/lib/lichen/state-<net>/validator-keypair.json"
echo ""
...
echo ""
echo "   3. Create genesis:"
echo "      Mainnet genesis refuses compiled market defaults. Either let lichen-genesis fetch live prices,"
echo "      or pass --genesis-prices-file with licn_usd_8dec, wsol_usd_8dec, weth_usd_8dec, and wbnb_usd_8dec."
echo "      sudo -u lichen HOME=$DATA_DIR LICHEN_HOME=$DATA_DIR LICHEN_CONTRACTS_DIR=$DATA_DIR/contracts \\"
echo "        lichen-genesis --network <net> --db-path $DATA_DIR/state-<net> \\"
echo "        --wallet-file $DATA_DIR/genesis-keys-<net>/genesis-wallet.json \\"
echo "        --initial-validator <PUBKEY_FROM_STEP_2> [--genesis-prices-file /path/to/genesis-prices.json]"
echo ""
