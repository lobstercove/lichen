...
signed `v0.5.228`; keep `v0.5.228` as the signed restart-safe anchor. Preserve
...
- Do not deploy a release that changes consensus rules with a mixed-version
  rolling restart. The current safe anchor `v0.5.228` must remain available
...
```bash
export LICHEN_RELEASE_TAG=v0.5.228
