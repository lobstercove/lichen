...
// Footer resource links now use data-lichen-app
const footerResourceCol = footer.match(/Resources[\s\S]*?<\/ul>/);
const footerResources = footerResourceCol ? footerResourceCol[0] : '';
assert(/data-lichen-app=["']developers["']/.test(footerResources), 'footer resources use data-lichen-app="developers"');

// Check that developer portal paths are used
assert(/data-lichen-path=["']\/architecture\.html["']/.test(footerResources) ||
    /data-lichen-path=["']\/getting-started\.html["']/.test(footerResources),
    'footer resources link to developer portal pages');

// Validator guide link in footer
assert(/data-lichen-path=["']\/validator\.html["']/.test(footerResources),
...
assert(/data-lichen-app=["']developers["']/.test(validatorHtml),
    'validator CTA uses data-lichen-app="developers"');
assert(/data-lichen-path=["']\/validator\.html["']/.test(validatorHtml),
...
assert(/class LichenWS/.test(explorerJs), 'explorer.js defines LichenWS');
...
assert(/LICHEN_CONFIG/.test(sharedConfig), 'shared-config.js defines LICHEN_CONFIG');
assert(/explorer/.test(sharedConfig), 'LICHEN_CONFIG has explorer');
assert(/wallet/.test(sharedConfig), 'LICHEN_CONFIG has wallet');
assert(/developers/.test(sharedConfig), 'LICHEN_CONFIG has developers');
assert(/faucet/.test(sharedConfig), 'LICHEN_CONFIG has faucet');
assert(/marketplace/.test(sharedConfig), 'LICHEN_CONFIG has marketplace');
assert(/dex/.test(sharedConfig), 'LICHEN_CONFIG has dex');

// data-lichen-app links exist in HTML
const lichenAppLinks = indexHtml.match(/data-lichen-app=["'][^"']+["']/g) || [];
assert(lichenAppLinks.length >= 6, `found ${lichenAppLinks.length} data-lichen-app links (expected >= 6)`);
...
assert(/class LichenRPC/.test(scriptJs), 'LichenRPC class exists');
