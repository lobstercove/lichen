...
LICHEN_BIN="${LICHEN_BIN:-$(pwd)/target/release/licn}"
...
# Also try stripping "token" suffix for wrapped token directories (lusd_token → musd)
...
if rpc_has_result "getMossStakePoolInfo" "[]"; then pass "getMossStakePoolInfo"; else fail "getMossStakePoolInfo"; fi
...
LICHENID_ID="$(find_contract_by_keyword "lichenid" "$CONTRACTS_RESULT")"
if [[ -n "$LICHENID_ID" ]]; then
  pass "LichenID contract discoverable"
  if rpc_has_result "getContractInfo" "[\"$LICHENID_ID\"]"; then
    pass "getContractInfo(lichenid)"
  else
    fail "getContractInfo(lichenid)"
  fi
else
  if [[ "$HAS_CONTRACT_NAMES" == "1" ]]; then
    fail "LichenID contract not discoverable from contract inventory"
  else
    pass "LichenID name-based discovery skipped (name metadata unavailable)"
  fi
fi

SPOREPUMP_ID="$(find_contract_by_keyword "sporepump" "$CONTRACTS_RESULT")"
if [[ -n "$SPOREPUMP_ID" ]]; then
  pass "SporePump launchpad contract discoverable"
  if rpc_has_result "getContractInfo" "[\"$SPOREPUMP_ID\"]"; then
    pass "getContractInfo(sporepump)"
  else
    fail "getContractInfo(sporepump)"
  fi
  if rpc_has_result "getProgramStats" "[\"$SPOREPUMP_ID\"]"; then
    pass "getProgramStats(sporepump)"
  else
    fail "getProgramStats(sporepump)"
  fi
else
  if [[ "$REQUIRE_LAUNCHPAD" == "1" && "$HAS_CONTRACT_NAMES" == "1" ]]; then
    fail "SporePump launchpad contract missing"
  else
    pass "SporePump name-based discovery skipped or optional"
  fi
fi
...
if [[ "$REQUIRE_TOKEN_WRITE" == "1" ]]; then
  if [[ -x "$LICHEN_BIN" && -f "$AGENT_KEYPAIR" && -n "$HUMAN_ADDR" ]]; then
    ...
    if "$LICHEN_BIN" --rpc-url "$RPC_URL" token create "$TOKEN_NAME" "$TOKEN_SYMBOL" --wasm "$TOKEN_WASM" --decimals 9 --keypair "$AGENT_KEYPAIR" >/tmp/e2e-token-create.log 2>&1; then
      pass "token create"
    else
...
PY
    ...
    if [[ -n "$TOKEN_ADDR" ]]; then
      ...
      if "$LICHEN_BIN" --rpc-url "$RPC_URL" token mint "$TOKEN_ADDR" 100 --to "$HUMAN_ADDR" --keypair "$AGENT_KEYPAIR" >/tmp/e2e-token-mint.log 2>&1; then
        pass "token mint"
      else
    ...
    fail "token write-path prerequisites missing (licn binary/agent keypair/human address)"
  fi
else
...
if [[ -n "$LICHENID_ID" ]]; then
  if rpc_has_result "getProgramStats" "[\"$LICHENID_ID\"]"; then
    pass "getProgramStats(lichenid)"
  else
    fail "getProgramStats(lichenid)"
  fi

  if rpc_has_result "getContractEvents" "[\"$LICHENID_ID\", 50, 0]"; then
    pass "getContractEvents(lichenid)"
  else
    fail "getContractEvents(lichenid)"
  fi
fi
...
if command -v cargo >/dev/null 2>&1; then
  if (
    cd "$ROOT_DIR" &&
      cargo test --release -p lichen-core processor::tests::test_ecosystem_grant_requires_multisig -- --exact >/dev/null 2>&1 &&
      cargo test --release -p lichen-core processor::tests::test_governed_proposal_lifecycle -- --exact >/dev/null 2>&1
  ); then
  ...
  if (
    cd "$ROOT_DIR" &&
      cargo test --release -p lichen-validator keypair_loader::tests::test_keypair_rotation_changes_loaded_pubkey -- --exact >/dev/null 2>&1 &&
      cargo test --release -p lichen-custody tests::test_master_seed_rotation_changes_derived_addresses -- --exact >/dev/null 2>&1
  ); then
