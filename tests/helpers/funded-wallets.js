...
async function fundAccount(address, amountLicn = 10, rpcUrl = 'http://127.0.0.1:8899', faucetUrl = 'http://127.0.0.1:9100') {
  ...
  function requestJson(url, body, pathOverride = null) {
    return new Promise((resolve, reject) => {
      const client = url.protocol === 'https:' ? https : http;
      const port = url.port || (url.protocol === 'https:' ? 443 : 80);
      const reqPath = pathOverride || `${url.pathname || '/'}${url.search || ''}`;
  ...
  function rpcCall(method, params) {
    ...
    return requestJson(new URL(rpcUrl), body);
  }
  ...
  function faucetCall(addr, amt) {
    ...
    return requestJson(url, body, `${basePath}/faucet/request`);
  }
