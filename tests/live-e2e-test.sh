...
echo ""
echo "================================================================"
echo "  LICHEN LIVE E2E TEST SUITE"
echo "  $(date)"
echo "================================================================"
echo ""
...
if [[ -n "$TEST_ADDR" ]]; then
    ...
    assert_result_or_ratelimit "requestAirdrop (10 LICN)" "$AIRDROP_RESULT"
...
if isinstance(data, dict):
    print(data.get('spores', data.get('balance', 0)))
else:
    print(data)
" 2>/dev/null || echo "0")
...
assert_eq "chain_id" "$CHAIN_ID" "lichen-testnet-1"
...
BASE_FEE=$(echo "$FC" | python3 -c "import sys,json; print(json.load(sys.stdin)['result'].get('base_fee_spores', -1))" 2>/dev/null || echo "-1")
assert_eq "base_fee_spores (1000000)" "$BASE_FEE" "1000000"
...
# ---- Section 20: MossStake Pool ----
echo ""
echo "--- 20. MOSSSTAKE POOL ---"
RSP=$(rpc "$RPC1" "getMossStakePoolInfo" "[]")
assert_result "getMossStakePoolInfo" "$RSP"

# ---- Section 21: LichenID RPC + Phase G Observability ----
echo ""
echo "--- 21. LICHENID RPC + PHASE G OBSERVABILITY ---"

MID_STATS=$(rpc "$RPC1" "getLichenIdStats" "[]")
assert_result "getLichenIdStats" "$MID_STATS"
...
if (( MID_TOTAL_IDENTITIES >= 0 )); then
    echo "  PASS  lichenid total_identities parsed ($MID_TOTAL_IDENTITIES)"
    ((PASS++))
else
    echo "  FAIL  lichenid total_identities parse failed"
    ((FAIL++))
    ERRORS="$ERRORS\n  FAIL: lichenid total_identities parse failed"
fi

MID_DIR=$(rpc "$RPC1" "getLichenIdAgentDirectory" '[{"limit":25,"offset":0}]')
assert_result "getLichenIdAgentDirectory" "$MID_DIR"
...
if (( MID_DIR_COUNT >= 0 && MID_DIR_TOTAL >= 0 )); then
    echo "  PASS  lichenid directory count/total parsed (count=$MID_DIR_COUNT, total=$MID_DIR_TOTAL)"
    ((PASS++))
else
    echo "  FAIL  lichenid directory count/total parse failed"
    ((FAIL++))
    ERRORS="$ERRORS\n  FAIL: lichenid directory count/total parse failed"
fi
...
if [[ -n "$MID_ADDR" ]]; then
    MID_ID=$(rpc "$RPC1" "getLichenIdIdentity" "[\"$MID_ADDR\"]")
    assert_result "getLichenIdIdentity(first directory agent)" "$MID_ID"

    MID_REP=$(rpc "$RPC1" "getLichenIdReputation" "[\"$MID_ADDR\"]")
    assert_result "getLichenIdReputation(first directory agent)" "$MID_REP"

    MID_SKILLS=$(rpc "$RPC1" "getLichenIdSkills" "[\"$MID_ADDR\"]")
    assert_result "getLichenIdSkills(first directory agent)" "$MID_SKILLS"

    MID_VOUCHES=$(rpc "$RPC1" "getLichenIdVouches" "[\"$MID_ADDR\"]")
    assert_result "getLichenIdVouches(first directory agent)" "$MID_VOUCHES"

    MID_ACH=$(rpc "$RPC1" "getLichenIdAchievements" "[\"$MID_ADDR\"]")
    assert_result "getLichenIdAchievements(first directory agent)" "$MID_ACH"

    MID_PROFILE=$(rpc "$RPC1" "getLichenIdProfile" "[\"$MID_ADDR\"]")
    assert_result "getLichenIdProfile(first directory agent)" "$MID_PROFILE"
    ...
    if [[ "$MID_AVAIL_NAME" == "available" || "$MID_AVAIL_NAME" == "busy" || "$MID_AVAIL_NAME" == "offline" ]]; then
        echo "  PASS  lichenid profile availability_name valid ($MID_AVAIL_NAME)"
        ((PASS++))
    else
        echo "  FAIL  lichenid profile availability_name invalid ($MID_AVAIL_NAME)"
        ((FAIL++))
        ERRORS="$ERRORS\n  FAIL: lichenid availability_name invalid"
    fi
    ...
    if [[ -n "$MID_TIER_NAME" ]]; then
        echo "  PASS  lichenid reputation tier_name present ($MID_TIER_NAME)"
        ((PASS++))
    else
        echo "  FAIL  lichenid reputation tier_name missing"
        ((FAIL++))
        ERRORS="$ERRORS\n  FAIL: lichenid tier_name missing"
    fi

    MID_REVERSE=$(rpc "$RPC1" "reverseLichenName" "[\"$MID_ADDR\"]")
    if echo "$MID_REVERSE" | python3 -c "import sys,json; d=json.load(sys.stdin).get('result'); assert d is None or isinstance(d, dict)" 2>/dev/null; then
        echo "  PASS  reverseLichenName shape valid"
        ((PASS++))
    else
        echo "  FAIL  reverseLichenName shape invalid"
        ((FAIL++))
        ERRORS="$ERRORS\n  FAIL: reverseLichenName shape invalid"
    fi
    ...
    if [[ -n "$MID_NAME" ]]; then
        MID_RESOLVE=$(rpc "$RPC1" "resolveLichenName" "[\"$MID_NAME\"]")
        assert_result "resolveLichenName(reverse name)" "$MID_RESOLVE"
    ...
    assert_result "batchReverseLichenNames(mixed existing/missing)" "$MID_BATCH"
else
    echo "  SKIP  no identities in agent directory for per-identity LichenID checks"
    ((SKIP+=12))
fi
...
if [[ -n "${LICHENID_G_PHASE_WRITE_TESTS:-}" ]]; then
    echo "  PASS  Phase G write-tests gate enabled (LICHENID_G_PHASE_WRITE_TESTS set)"
    ((PASS++))
else
