...
async function discoverContracts() {
    ...
    const symbolToToken = {
        'LICN': 'LICN', 'LUSD': 'lUSD', 'WSOL': 'wSOL', 'WETH': 'wETH', 'MOSS': 'MOSS',
    };
...
async function runTests() {
    ...
    assert(balance.spendable > 0, `Alice spendable: ${balance.spendable_licn} LICN (${balance.spendable} spores)`);
    ...
    // ── Test 6: LICN transfer (Alice → Bob) via sendTransaction ──
    console.log('\n── Test 6: LICN Transfer (Alice → Bob) ──');
    {
        ...
        const transferAmount = 1_000_000_000; // 1 LICN in spores
        ...
        try {
            ...
            assert(bobBal.spendable > 0, `Bob spendable after transfer: ${bobBal.spendable_licn} LICN`);
        } catch (e) {
    ...
    console.log('\n── Test 7: DEX place_order via Contract Call ──');
    {
        ...
        const pairId = 1;   // LICN/lUSD (first genesis pair)
        const side = 'buy';
        const orderType = 'limit';
        const price = Math.round(0.10 * PRICE_SCALE);   // $0.10
        const quantity = Math.round(1.0 * PRICE_SCALE);  // 1 LICN
        ...
        try {
            ...
            // May fail if Alice doesn't have lUSD for the buy side — that's expected
    ...
    {
        try {
            const musdBal = await rpc('getTokenBalance', [TOKEN_CONTRACTS.lUSD, alice.address]);
            assert(typeof musdBal.balance === 'number', `Alice lUSD balance: ${musdBal.ui_amount} (${musdBal.balance} raw)`);
        } catch (e) {
    ...
    console.log('\n── Test 13: Multi-instruction Transaction ──');
    {
        ...
        const amount1 = 100_000_000; // 0.1 LICN
        const amount2 = 200_000_000; // 0.2 LICN
