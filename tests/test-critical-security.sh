...
if [ -f "$DEX_JS" ]; then
  # All localStorage keys should have a prefix like "dex" or "licn"
  check "localStorage keys namespaced" \
    "grep 'localStorage' '$DEX_JS' | grep -q 'dex\|licn'"
else
...
MKT_CONTRACT="$ROOT/contracts/lichenmarket/src/lib.rs"
...
check "CON-01: Oracle uses 9_000 slot threshold" \
  "grep -q '9_000' '$ROOT/contracts/lichenoracle/src/lib.rs'"

# CON-05: sporepump returns false (not true) when unconfigured
check "CON-05: sporepump rejects unconfigured LICN" \
  "grep -A3 'LICN token address not configured' '$ROOT/contracts/sporepump/src/lib.rs' | grep -q 'return false'"

# CON-06: thalllend u128 cast
check "CON-06: thalllend u128 overflow fix" \
  "grep -q 'as u128.*LIQUIDATION\|deposit as u128' '$ROOT/contracts/thalllend/src/lib.rs'"

# CON-07: lichendao PROPOSAL_SIZE = 212
check "CON-07: PROPOSAL_SIZE = 212" \
  "grep -q 'PROPOSAL_SIZE.*=.*212' '$ROOT/contracts/lichendao/src/lib.rs'"
echo ""
