...
assert(browseJs.includes("escapeHtml(priceInLicn)"), 'M-3.5  list view price escaped');
...
assert(marketCss.includes('background: var(--teal-primary)'), 'M-11.1 btn-primary uses flat teal (no gradient)');
assert(marketCss.includes('background: var(--teal-dark)'), 'M-11.2 btn-primary hover uses teal-dark');
...
assert(marketCss.includes('border-color: var(--teal-primary)'), 'M-11.5 btn-secondary hover uses teal border');
...
assert(!createHtml.includes('~0.001 LICN'), 'M-12.5 No hardcoded gas amount in HTML');
assert(createHtml.includes('0.500 LICN'), 'M-12.6 Total cost is 0.500 LICN (no gas)');
...
const contractRs = fs.readFileSync(path.join(__dirname, '..', 'contracts', 'lichenmarket', 'src', 'lib.rs'), 'utf8');
...
assert(!createJs.includes('GAS_ESTIMATE'), 'M-27.3  No GAS_ESTIMATE (no gas on Lichen)');
...
assert(createJs.includes('storeMetadataOnMoss'), 'M-27.13 Metadata storage routed through MOSS helper');
assert(createJs.includes("buildContractCallData('store_data'"), 'M-27.14 MOSS store_data contract call used for metadata persistence');
...
assert(createJs.includes('program_id: mossProgram'), 'M-28.8  create.js moss storage call uses resolved program id');
...
assert(marketplaceDataJs.includes("rpcCall('getLichenMarketStats'"), 'M-34.1  getStats uses backend getLichenMarketStats');
...
assert(itemJs.includes('lichenmarket_favorites_v1'), 'M-35.8  item.js persists favorites in wallet-scoped storage');
...
const lichenpunksRs = fs.readFileSync(path.join(__dirname, '..', 'contracts', 'lichenpunks', 'src', 'lib.rs'), 'utf8');
assert(lichenpunksRs.includes('caller.0 == minter.0 || caller.0 == to.0'), 'M-36.1  lichenpunks mint allows minter or self-mint caller');
...
assert(contractRs.includes('MIN_OFFER_PRICE'), 'M-37.1  lichenmarket defines minimum offer floor constant');
assert(contractRs.includes('MAX_ACTIVE_OFFERS_PER_WALLET'), 'M-37.2  lichenmarket defines per-wallet active offer limit constant');
assert(contractRs.includes('offerer_active_count_key') && contractRs.includes('reserve_offer_slot_if_needed'), 'M-37.3  lichenmarket tracks/reserves active offer slots per wallet');
