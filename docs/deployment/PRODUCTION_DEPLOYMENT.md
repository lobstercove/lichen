...
Current signed-release target for this runbook is `v0.5.188`; keep `v0.5.187` as the signed rollback point. The GitHub Release archive must be installed on all four VPSes, the seed creates genesis, and `seed-02`, `seed-03`, plus `seed-04` start from empty chain state and join from peers without a state copy. A passing verifier must report all four validators healthy with bridge `4/2`, oracle feeds including BTC, empty faucet history, 32 manifest symbols, and the mandatory 13 DEX CLOB pairs, AMM pools, and router routes including `wBTC/lUSD` and `wBTC/LICN`. `v0.5.188` changes consensus leader selection and must be installed with a coordinated stop/install/start rollout so validators do not run mixed leader schedules. Use `scripts/rolling-release-deploy.sh` only for later non-consensus code-only updates; do not flush state for that path.
