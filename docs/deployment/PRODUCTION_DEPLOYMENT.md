...
The target testnet release for this runbook is `v0.5.277`; the installed signed
fleet release is `v0.5.274`, and the immediate restart-safe anchor is
`v0.5.265`. Install `v0.5.277` only after
its exact tag workflow, attestations, checksums, detached PQ signature, and
four-validator Archive V2 gate pass. Once the fleet is proven on `v0.5.277`,
keep only `v0.5.277` and `v0.5.265` installed on each validator. Historical
...
```bash
LICHEN_RELEASE_TAG=v0.5.277 LICHEN_COORDINATED_RELEASE=1 \
  bash scripts/rolling-release-deploy.sh testnet
```
