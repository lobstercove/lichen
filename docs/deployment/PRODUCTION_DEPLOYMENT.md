...
The target testnet release for this runbook is `v0.5.273`; the installed signed
fleet release is `v0.5.272`, and the immediate restart-safe anchor is
`v0.5.265`. Install `v0.5.273` only after
its exact tag workflow, attestations, checksums, detached PQ signature, and
four-validator Archive V2 gate pass. Once the fleet is proven on `v0.5.273`,
keep only `v0.5.273` and `v0.5.265` installed on each validator. Historical
...
For the v0.5.273 preserved-chain Testnet DEX repair, use the single coordinated
release entrypoint only after the exact signed release has passed every gate:

```bash
LICHEN_RELEASE_TAG=v0.5.273 \
...
database. The repair dry-runs first, requires the v0.5.273 confirmation, and
...
```bash
LICHEN_RELEASE_TAG=v0.5.273 LICHEN_COORDINATED_RELEASE=1 \
  bash scripts/rolling-release-deploy.sh testnet
```
