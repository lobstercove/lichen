...
Current signed-release target for this runbook is `v0.5.194`; keep `v0.5.193` as the signed rollback point. The GitHub Release archive must be installed on all four VPSes, the seed creates genesis, and `seed-02`, `seed-03`, plus `seed-04` start from empty chain state and join from peers without a state copy. A passing verifier must report all four validators healthy with bridge `4/2`, oracle feeds including BTC, empty faucet history, 32 manifest symbols, and the mandatory 13 DEX CLOB pairs, AMM pools, and router routes including `wBTC/lUSD` and `wBTC/LICN`. `v0.5.194` keeps the `v0.5.193` deterministic MossStake/checkpoint resume baseline and adds immediate full-block fallback when compact blocks arrive before all referenced transactions, so lagging validators do not wait for periodic catch-up and miss proposer windows. All validators must end on the same signed release before the rollout is considered complete. Use `scripts/rolling-release-deploy.sh` only for non-consensus code-only updates; do not flush state for that path.
...
export LICHEN_RELEASE_TAG=v0.5.194
