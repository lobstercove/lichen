...
The target testnet release for this runbook is `v0.5.270`; the current signed
...
four-validator Archive V2 gate pass. Once the fleet is proven on `v0.5.270`,
...
For the v0.5.270 preserved-chain Testnet DEX repair, use the single coordinated
release entrypoint only after the exact signed release has passed every gate:

```bash
LICHEN_RELEASE_TAG=v0.5.270 \
...
database. The repair dry-runs first, requires the v0.5.270 confirmation, and
...
```bash
LICHEN_RELEASE_TAG=v0.5.270 LICHEN_COORDINATED_RELEASE=1 \
  bash scripts/rolling-release-deploy.sh testnet
```
