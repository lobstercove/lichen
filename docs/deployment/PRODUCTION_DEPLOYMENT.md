...
When `LICHEN_RELEASE_TAG` is set, the script downloads the GitHub Release archives, verifies `SHA256SUMS`, installs `lichen-validator`, `lichen-genesis`, `lichen`, `zk-prove`, `lichen-custody`, `lichen-faucet`, and bundled `seeds.json` from those archives, and verifies the installed binary hashes. It must not overwrite those runtime binaries from any remote `target/release` directory.
