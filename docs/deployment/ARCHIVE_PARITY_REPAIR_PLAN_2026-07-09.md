...
| AP-077 | Prevent target validation backpressure and SSH connection limiting from truncating fleet repair transport | Exact signed live dry run, immutable stream replay, bounded-page checksum and report guards | Fix and exact live dry runs complete through the latest transport boundary. The signed `bdc8de0d` two-hop stream failed closed twice, while its exact 34,657,898-byte export imported cleanly when sequenced. Bounded checksummed pages then passed an 18-report six-category smoke and one full 5,000-body page plus indexes: 15 reports, 92,928 rows, 15 matching transfer digests, and zero conflicts in 3 minutes 44 seconds. The first full EU-range attempt proved all 2,872,006 linked bodies but page 1 hit the fleet's six-new-SSH-connections-per-30-seconds UFW policy. No live database write occurred. The relay now creates one pinned persistent target control each; a three-page live dry run passed 180,000 row checks with zero conflicts and complete transient cleanup. Replacement hosted gates/tag and the restarted complete-range dry run remain. |
