...
**Status:** Complete for the existing testnet release. Public archive storage is an automatic non-development testnet/mainnet invariant. The available source-backed union is replicated conflict-free across US/EU/SEA/IN through stopped tip `9,771,064`, and the resulting 20-category composite manifest matches root `71f3da66...6f46` on all four. Signed `v0.5.225` is published and runs on all four validators with exact binary/unit parity, zero systemd restarts, and fresh production. Exact source-backed repair closed the former incomplete slot `5,276,000`; only owner-approved irrecoverable signed bodies `2,872,006..4,298,999` remain an explicit exception for existing `lichen-testnet-1`, and no data was synthesized. Fresh networks and mainnet remain fail-closed and receive no waiver. Current 200 GB VPS capacity is adequate only for near-term testnet operation, not mainnet or indefinite archive growth.
...
```bash
# Define once in the guarded operator shell. Direct `sudo -u lichen` inherits a
# 1,024-descriptor interactive limit on the current VPSes and is not valid for
# archive scans.
run_validator_admin() {
  local binary="$1"
  shift
...
run_validator_admin /tmp/lichen-validator-0.5.224-candidate \
  --network testnet \
...
- Any same-key semantic mismatch in repaired public-history column families
  must abort execution and produce evidence. The only permitted replacement is
  source-backed completion of a header-only block whose canonical key/hash,
  full header, transaction Merkle root, and oracle payload all match; the target
  validator's local finality certificate is preserved.
...
- skip existing semantically identical rows, including block bodies that differ
  only in their valid local commit-certificate subset
- complete only a strictly matching header-only block and report it separately
- abort on every other existing conflict
...
- produce dry-run and execute evidence with inserted, upgraded-incomplete,
  skipped, and conflicted row counts
...
# Binary framing and persistent SSH controls are automatic for every category.
bash scripts/stream-public-history-repair.sh --execute --leave-target-stopped \
  --source SOURCE --targets "TARGETS" \
...
`9,236,790..9,237,999`, and retained only the canonical header at `9,238,000`.
The `9,238,000` header and block hash match US/EU/IN, but SEA has an empty
transaction vector against a nonempty transaction root. SEA matches the other
validators again from `9,238,001`. US, EU, and IN are verified sources for the
inclusive bounded range `9,236,790..9,238,000`. Before final live archive
parity, SEA must be stopped and repaired additively from one of those sources
through the guarded stream repair path, then all four must pass the fixed-tip
offline manifest gate. The repair must not copy another validator database or
synthesize a block body.
...
SEA's recent `9,236,790..9,237,999` hole plus the header-only `9,238,000`
boundary, before fixed-tip parity can pass.
Evidence is under `evidence/archive-parity/testnet-20260716T022033Z`.

## 2026-07-16 Signed Artifact And SEA Boundary Preflight

Commit `ce979e6deb0de2d53091e8d3dd715b26b41f99d7` was temporarily tagged as
`v0.5.224` after the AP-069 correction. Hosted CI run `29501871873` passed all
15 jobs. Release workflow `29506127542` produced a draft whose checksums,
GitHub attestations, detached post-quantum checksum signature, tag binding, and
five assets all verified. The draft was never published, and its Linux binary
was staged only under `/tmp` for read-only preflight; no candidate was installed
or executed by systemd.

That exact binary proved automatic cold-store discovery against cold-only slots
on US, EU, SEA, and IN. A broader live read-only pass then found the SEA boundary
condition missed by the earlier sampled probe. SEA slot `9,238,000` has the same
canonical header and block hash as US/EU/IN, but its stored block has no
transactions despite a nonzero transaction root. SEA is complete again at
`9,238,001`. A source-proven block dry run from US over the inclusive range
`9,236,790..9,238,000` reported 1,211 source rows, 1,210 insertions, and one
conflict at the header-only boundary. Evidence is under
`evidence/archive-parity/testnet-20260716T-coordinated-v05224-prestop` and
`evidence/public-history-stream-repair/testnet-20260716T-sea-block-boundary-dry-run`.
This supersedes commit `ce979e6d`, its tag, and its draft artifacts.

AP-076 implements one narrow, source-backed completion rule. The incoming block
must be a full body whose key/hash, complete canonical header, transaction
Merkle root, and oracle payload match the target header-only row. The target row
must have no transactions, no fee vector, and a nonzero transaction root. The
repair preserves the target validator's local commit round and certificate
subset, writes and WAL-syncs the completed body to the target cold store, then
deletes and WAL-syncs the hot placeholder. Every other same-key difference
...
The first complete EU-range attempt then passed the exact `0..2,872,005`
source proof and page 0, but page 1 failed closed when cleanup, copy, and promote
opened enough fresh target sessions to trigger every VPS's UFW limit of six new
SSH connections in 30 seconds. Fail2ban had not banned EU and no live database
write occurred. The corrected relay establishes one pinned source-to-target control
master per target, reuses it for every bounded page operation, checks it before
each copy, and closes/removes it on every exit. A three-page live dry run crossed
the prior failure boundary with 180,000 target-row checks, 175,944 missing rows,
4,056 identical rows, zero conflicts, and no remaining transient files.
...
frontend/wallet/deployment QA, shell syntax, and all 12 local helper guards
pass.

The exact final-source four-validator gate joined V2 through V4 from empty
hot/cold stores, recovered a 140-slot LiveSync pause in-process, restarted V4
and V1 from their own state, resumed all four from one preserved tip, and
matched canonical certificates. Checkpoint-1000 manifests matched root
`da5e1db3c5d6e196800bf7ea5ecea29ef59cffc6fe29c2b15dd9f0a048079958`.
Volume/user journeys passed 140/140 and launchpad/governance/graduation passed
104/104. The post-journey checkpoint-3000 manifest root matched on all four:
`828d5e8f89f460e1f0b3240a1e2fa3a38aab4add9525b31816bcf9db48d444a5`.
Transcript:
`evidence/local-multi-validator/20260716T181722Z-ap076-final-4/transcript.log`,
SHA-256 `18bd616fedb59f88040361bc2308ba0f6d6dd2373bbb27b724a358073063f4be`.

The independent final-source ten-validator gate joined V2 through V10 from
empty stores, recovered V10 in-process after the same 140-slot gap, restarted
V10 and V1 from their own state, resumed all ten from one preserved tip,
continued finality with V9 and V10 offline, and recovered both with preserved
identities. Canonical certificates matched at slots `2598` and `2690`; every
validator's checkpoint-3000 hot/cold manifest matched root
`a8d3b9247b782fdd2cc5a051d8fbfbe65d12decff21a1b0be160abaa7c1cf0bc`.
Transcript:
...
block-body upload amplification. A later 5,000-body fleet page proved the local
operator uplink, rather than validation, was the remaining bulk bottleneck. The
helper now uses a pinned one-shot `sshd -i` relay over the authenticated source
connection to forward exactly one loaded local agent identity, forces its
staged public key for source-to-target SSH, and transfers compressed pages
directly between VPSes. No private key is copied, no system SSH or authorized
key configuration changes, source/target SHA-256 and byte counts must match,
target promotion is atomic, and relay/page files are removed on every exit.
The prior local bounded transfer remains the automatic no-agent fallback. A
live 11-slot six-category relay smoke produced 18 clean target reports and 18
matching transfer digests. A full 5,000-body page plus its bounded transaction
indexes then produced 15 clean target reports, 92,928 validated source rows,
...
live repair, and final fleet parity remained mandatory before AP-080/AP-081
could close; the 2026-07-21 execution below completed those gates.

## 2026-07-21 v0.5.225 Release and Fixed-Tip Closure

Signed tag `v0.5.225` at
`c7a67e6ab0045dac66928ac977e7e3cc2af5ac75` passed hosted CI run
`29758667263` and five-platform release run `29762623386`. The earlier release
attempt exposed a Windows-only conditional-compilation defect and produced no
published release; the final tag corrected it. The public Linux x86_64 archive
is `24a1b704...300abb`, and all four validators install and execute exact
validator SHA-256 `12e8a18b...eea2f` under tagged unit
`e8eda48d...d70d39`.

The coordinated rollout preserved every validator's own state, archive,
identity, validator key, peer state, consensus WAL, access configuration, and
signed rollback artifacts. IN encountered the proven parent-root condition at
`9,736,991`; authenticated child `9,736,992` authorized exactly one missing
producer-counter increment. The repair logged once, catch-up completed, and the
final coordinated restart at 04:02:21..23 UTC has zero systemd restarts and no
repeat root/parent mismatch, panic, database error, or lingering InitialSync.
This closes AP-081 on the signed fleet.

The fixed-tip parity run stopped all four at `9,771,064`. EU/SEA/IN produced
byte-identical 20-category manifests. US differed by exactly five rows in each
of `transactions`, `tx_by_slot`, `tx_to_slot`, and `account_txs`; bounded bucket
isolation located every row at formerly incomplete slot `5,276,000`. US held a
complete block with hash `fa084aab...4ad0` and contiguous range digest
`5abba948...7da2`. Guarded source-backed repair inserted 43 official rows,
upgraded two exact matching header-only rows, found eight identical rows, and
reported zero conflicts. A separate block-key-bound import inserted the five
`account_txs` rows per target. All official and account-history post dry runs
were idempotent with zero conflicts.

The resulting stopped-tip composite proof has state root
`e18a4f93...6a093` and 20-category manifest root `71f3da66...6f46` on all four.
Independent contiguous verification now matches for slot `5,276,000`, closing
that exception. The only remaining existing-`lichen-testnet-1` waiver is the
unavailable signed-body interval `2,872,006..4,298,999`; it remains explicit,
non-transferable, and invalid for mainnet or any fresh network. This closes
AP-060 for every available source-backed byte without synthesizing history.

AP-080 is also live and closed: the signed fleet remains active above its
10 GiB runtime floor, skips new checkpoints below the distinct 20 GiB creation
floor, and no derived hard-linked checkpoint can force the old retention
deadlock. Finite 200 GB storage remains a capacity blocker for mainnet and
indefinite testnet growth.

The verifier delay was operational amplification, not a new consensus fault.
It ran four multi-million-row walks sequentially, used 1,000-row chunks, and
also offered a moving-tip full scan that could not be a strict proof. The helper
...
| AP-035 | Add peer/fleet archive parity automation so a new joiner can prove archive readiness from any validator | `tests/local-multi-validator-test.sh` and `scripts/verify-testnet-archive-parity.sh` output | Done for local harness plus VPS fleet verifier. Normal fleet checks are bounded concurrent host/RPC/fixed-block probes. Explicit strict manifests reuse SSH controls, write durable server-side results, scan four hosts concurrently with 20,000-row chunks, and retain the fixed-tip stop/restart gate; live execution evidence is tracked under AP-060. |
...
| AP-050 | Run local four-validator testnet archive parity gate | `LICHEN_RUN_LAUNCHPAD_E2E=1 LICHEN_RUN_VOLUME_E2E=1 LICHEN_LOCAL_ARCHIVE_COLD=1 LICHEN_COLD_RETENTION_SLOTS=20 LICHEN_COLD_MIGRATION_INTERVAL_SECS=5 bash tests/local-multi-validator-test.sh 4` | Done on the exact AP-076 final source. V2/V3/V4 joined from empty stores; V4 recovered a 140-slot LiveSync pause in-process; V4 and V1 resumed their own state; all four resumed from one preserved tip; and canonical certificate parity matched. Checkpoint-1000 root `da5e1db3...079958`; volume/user journeys 140/140; launchpad/governance/graduation 104/104; final checkpoint-3000 root `828d5e8f...d444a5`. Transcript SHA-256 `18bd616f...063f4be`. The independent ten-validator gate also passed 8/10 finality, preserved-state recovery, certificate parity, and checkpoint-3000 root `a8d3b924...1cf0bc`; transcript SHA-256 `866ba875...0e4368f`. |
...
| AP-069 | Make archive-backed hot/cold retention an automatic public-network invariant | Runtime/admin flag rejection tests, cold-only handler regression, environment contracts, four-validator archive gate | Done and live in `v0.5.224`. Public network runtime/admin paths derive and open `archive-<network>` automatically; manual public archive mode/store flags are rejected. Cold-only handler, four-validator, ten-validator, exact Linux, live repair, and post-deployment EU migration evidence pass. Disposable `--dev-mode` clusters retain explicit controls, and independent repair sources use only `--source-cold-store`. |
| AP-071 | Prevent post-checkpoint LiveSync stalls and repair SEA's bounded July 15 archive gap | Same-process material-gap recovery, progress-watchdog tests, source range proof, fixed-tip fleet manifests | Done and live in `v0.5.224`. Material gaps and checkpoint-repair entry return to bounded InitialSync, and only canonical or verified snapshot progress feeds the watchdog. Local four-/ten-validator gates passed pause and preserved-state restart recovery. SEA `9,236,790..9,238,000` was repaired source-backed and included in the fixed-tip fleet manifests; EU then resumed its own preserved database and caught up to the fleet in under two minutes. |
...
| AP-076 | Complete an exactly matching header-only canonical block without weakening conflict checks | SEA boundary dry run, Merkle/header/oracle guards, local-certificate preservation, idempotence, all-category framed stream tests | Done and exercised live. The typed source-backed completion repaired SEA slot `9,238,000`, WAL-synced cold storage before retiring hot data, preserved SEA's local finality proof, and left all non-identical input conflict-aborting. The repaired row is included in the four-way fixed-tip manifest parity proof. |
