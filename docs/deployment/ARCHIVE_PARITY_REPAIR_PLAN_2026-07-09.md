...
**Status:** Complete for the existing testnet release. Public archive storage is an automatic non-development testnet/mainnet invariant. The source-backed union is replicated conflict-free across US/EU/SEA/IN through stopped tip `9,680,288`, and the final 20-category composite manifest matches root `be86ae12...d63d2e1` on all four. Signed `v0.5.224` is published and runs on all four validators with exact binary/unit parity, zero systemd restarts, and fresh production. The owner-approved irrecoverable bodies `2,872,006..4,298,999` and incomplete slot `5,276,000` remain explicit exceptions only for existing `lichen-testnet-1`; no data was synthesized. Fresh networks and mainnet remain fail-closed and receive no waiver. Current 200 GB VPS capacity is adequate only for near-term testnet operation, not mainnet or indefinite archive growth.
...
| AP-060 | Repair the current July chain on US, EU, SEA, and IN from verified source history | Per-range proof, per-host dry-run/execute evidence, fixed-tip manifests | Done for all available source-backed history. Additive, checksummed, conflict-aborting repairs replicated the EU/IN/US/SEA union, including slot `5,275,999` and SEA `9,236,790..9,238,000`, to all four validators. At stopped tip `9,680,288`, state root `6d871d43...6534457` and the final 20-category composite manifest root `be86ae12...d63d2e1` match on US/EU/SEA/IN. No audited source contains signed bodies `2,872,006..4,298,999`, and slot `5,276,000` remains incomplete; both are explicit, honest exceptions only for existing `lichen-testnet-1`. Mainnet/fresh-network startup, joining, checkpoint, and snapshot import remain fail-closed on any absent, behind, or mismatched genesis-to-tip proof. |
| AP-065 | Recover four-validator live production and prove fresh-tip liveness | Same current slot/hash, fresh block age, all four producers observed | Done on signed `v0.5.224`. US/EU/SEA/IN are active and enabled, execute validator SHA-256 `4fe73939...195f9`, use tagged unit SHA-256 `e8eda48d...70d39`, have no nested supervisor, and report zero systemd restarts. The 2026-07-20 post-maintenance sample observed four fresh producers, tips `9,682,977..9,682,998`, public health at `9,683,000`, and a 334 ms observed block interval. |
