...
**Status:** Public archive storage is an automatic non-dev testnet/mainnet invariant; EU/IN capacity recovery and redundant public RPC ingress are complete; the owner accepted the irrecoverable AP-060 gap for this existing testnet only, while mainnet remains fail-closed. AP-076 supersedes signed draft commit `ce979e6d`: SEA slot `9,238,000` is a matching canonical header with a missing body, and the old repair path treated it as an unrepairable conflict. The guarded completion fix passes the complete local source, dependency, contract, frontend, four-validator, and ten-validator release gates. AP-077 then found that audited `sudo` could duplicate repair payloads into its I/O log; the corrected direct and local-bounded smoke paths keep bulk bytes inside the unprivileged child. Replacement commit/tag, hosted CI and signed artifacts, restarted complete-range dry run, stopped-fleet source-union repair, coordinated activation/deployment, fixed-tip live parity, and publication remain. US/EU/SEA/IN still run `v0.5.223`; EU uses the audited exact-tag sparse-maintenance bridge while the other three use the signed rollback artifact.
...
| AP-071 | Prevent post-checkpoint LiveSync stalls and repair SEA's bounded July 15 archive gap | Same-process material-gap recovery, progress-watchdog tests, source range proof, fixed-tip fleet manifests | Code and exact final-source local gates done; live repair is part of AP-070 deployment. Material gaps and every checkpoint-repair entry return to bounded InitialSync, and only canonical or verified snapshot progress feeds the watchdog. The AP-076 four- and ten-validator gates both passed the 140-slot same-process pause, own-state/coordinated restarts, and identical checkpoint-3000 manifests. SEA range `9,236,790..9,237,999` plus its header-only boundary at `9,238,000` remain to be additively repaired from US/EU/IN before final live parity. |
...
| AP-078 | Keep canonical transaction/index repair pagination linear in the selected source range | Later-row isolation regression, full Core suite, strict Clippy, exact Linux live timing | Fixed in source. The exact `373db78c` live dry run showed every 20,000-row page continued through the entire remaining `tx_by_slot` archive after its canonical page was already full. The merge now stops at that page's canonical cursor frontier and skips duplicate index lookups already backed by the selected block bodies, while retaining source-backed index-only rows before the frontier. A malformed later-row regression proves an early page never reads beyond that bound. Replacement hosted artifact and exact live-range dry run remain. |
| AP-070 | Cut new tag and publish release artifacts | Tag, checksums, crates/npm status | The `373db78c` `v0.5.224` hosted CI, release workflow, strict four-validator gate, artifacts, checksums, attestations, and detached signature passed. Its unpublished draft/tag is superseded by AP-078 before any deployment or registry publication. Pending: replacement commit/tag and hosted gates, exact Linux dry-run evidence, complete source-backed fleet-union repair, coordinated deployment/activation, fixed-tip fleet parity, and package publication. Mainnet release remains prohibited until a fresh chain proves complete genesis-to-tip archives without any waiver. |
