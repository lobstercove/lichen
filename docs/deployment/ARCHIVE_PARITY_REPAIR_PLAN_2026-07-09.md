...
| AP-074 | Make the hosted archive gate self-contained instead of relying on ignored workstation test files | Clean tag checkout, recursive CommonJS dependency closure, Git tracking/ignore assertions, exact four-validator rerun | Done locally. The first `v0.5.224` tag attempt passed consensus recovery, certificate parity, and four-node hot/cold manifest parity, then correctly failed before volume journeys because `tests/e2e-volume.js` was absent from the clean checkout. A prior repository cleanup had ignored and untracked `tests/`; only the shell harness was later force-added. The volume and launchpad suites plus their five helper modules are now explicit tracked release inputs. `test_archive_parity_gate_assets.js` recursively verifies every local `require`, file existence, Git tracking, ignore policy, workflow flags, and harness invocation in both branch and release CI. The clean-closure rerun passed same-process 140-slot recovery, own-state and coordinated restarts, 140/140 volume journeys, 104/104 launchpad/governance/graduation journeys, and four-node checkpoint-3000 manifest root `7609ff7a...6bc1376`; transcript SHA-256 `eac37545...e5fd9f66`. No release artifact was created from the failed tag attempt. |
