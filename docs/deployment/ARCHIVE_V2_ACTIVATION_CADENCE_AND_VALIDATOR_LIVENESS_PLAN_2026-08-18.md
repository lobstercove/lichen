...
`v0.5.276` is the only successor candidate: it serializes that admission
against canonical block application, pauses cold migration until admission
succeeds, and makes any failure synchronous and nonzero.
...
`v0.5.276`. Once `v0.5.276` is running and four-way V2 parity plus rollback
...
approved for indefinite archive growth, and `v0.5.276` still requires focused and
...
- the transition is reversible through the preserved baseline environment and
  ...
  audit history, not installed rollback binaries after `v0.5.276` acceptance.
...
`v0.5.265` introduced the role-bootstrap correction, and `v0.5.276` carries it
...
2. Preserve the completed `v0.5.270` clean and hosted runtime evidence, the
   ...
   and its exact failed immutable tag log. Qualify the narrow `v0.5.276`
   deferred-admission serialization through the same fresh full-archive join.
3. Commit through protected `main`, create the immutable `v0.5.276` tag, wait
...
4. Capture a stopped preflight on all four hosts, prove identities/WALs/state,
   preserve `v0.5.265`, install only the signed `v0.5.276` workflow artifact, and perform
...
9. Publish and deploy the matching wallet `0.1.9`, exchange
   `exchange-testnet-v0.5.276`, developer portal, README, and frontend surfaces
