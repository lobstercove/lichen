...
**Release candidate:** `v0.5.218`; keep `v0.5.215` as the rollback anchor until a newer signed rollback point is explicitly recorded.
...
- Core protocol crates are at `0.5.218`: `core`, `rpc`, `validator`, and `cli`.
- The root `README.md` and production deployment runbook now describe `v0.5.218` as the recovery candidate while keeping `v0.5.215` as the rollback anchor.
...
- `sdk/rust/Cargo.toml` package version is `0.1.5` and now depends on `lobstercove-lichen-core = "=0.5.218"` while using the local `../../core` path.
