...
| P1-05 | Developer portal exchange page aligned | Done | No | Local source page and hub links exist; the portal now carries inline exchange metadata, deposit/withdrawal cookbooks, finality/archive policy, operations contacts, validation gates, mainnet handoff, and release-tagged source links; `bash scripts/deploy-cloudflare-pages.sh developers` deploys this page and the public readiness gate requires the inline cookbook snippets |
...
| Public readiness gate script | `scripts/qa/exchange_public_readiness.py` writes `tests/artifacts/exchange-public-readiness-report.json`, requires the exchange developer-page content snippets including `Deposit Cookbook`, `Withdrawal Cookbook`, `Canonical JSON-RPC Cookbook`, `Mainnet Handoff`, and `testnet-only`, honors `--scope testnet` versus `--scope full`, checks rollback anchor `v0.5.221`, and fails closed while blocking public endpoint, developer portal, status approval, or release-selection gates remain open |
