...
| Testnet RPC URL | `https://testnet-rpc.lichen.network` | Reachable but not exchange-ready on 2026-06-30: `health.status = behind`, `reason = stale_tip`, slot `6715444`; recovery is pending signed `v0.5.217` rollout | `seeds.json`, `core/src/network.rs`, `developers/shared-config.js`, tracker Phase 5 metadata evidence |
...
| --- | --- | --- |
| M-02 | Runtime fee value not publicly verified on the current live public target | `getFeeConfig` result from a healthy public target network after `v0.5.217` recovery rollout; 2026-06-30 public testnet is readiness-gated by stale tip |
