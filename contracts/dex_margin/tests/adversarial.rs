...
fn test_add_margin_u64_overflow() {
    ...
    assert_eq!(
        add_margin(trader.as_ptr(), 1, u64::MAX),
        6,
        "overflow should return error 6"
    );
}
...
fn test_remove_margin_to_below_maintenance() {
    ...
    assert_eq!(
        result, 6,
        "removing margin below maintenance should be rejected"
    );
}
...
fn test_add_margin_to_closed_position() {
    ...
    assert_eq!(close_position(trader.as_ptr(), 1), 0);
    assert_eq!(
        add_margin(trader.as_ptr(), 1, 100),
        3,
...
fn test_set_mark_price_zero() {
    let admin = setup();
    assert_eq!(
        set_mark_price(admin.as_ptr(), 1, 0),
        2,
...
fn test_flash_pump_then_liquidate_short() {
    ...
    assert_eq!(
        liquidate(liquidator.as_ptr(), 1),
        0,
        "should be liquidatable after pump"
    );
...
fn test_set_max_leverage_zero() {
    let admin = setup();
    assert_eq!(
        set_max_leverage(admin.as_ptr(), 1, 0),
        2,
        "zero max leverage should be rejected"
    );
}
...
fn test_set_max_leverage_above_100() {
    let admin = setup();
    assert_eq!(
        set_max_leverage(admin.as_ptr(), 1, 101),
        2,
        "leverage > 100 should be rejected"
    );
    // 100 should succeed
    assert_eq!(
        set_max_leverage(admin.as_ptr(), 1, 100),
        0,
        "leverage = 100 should be accepted"
    );
}
...
fn test_custom_max_leverage_enforced() {
    ...
    // 4x should fail
    assert_eq!(
        open_position(trader.as_ptr(), 1, 0, 1000, 4, 200),
        2,
