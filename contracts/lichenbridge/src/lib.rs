// LichenBridge — Cross-Chain Bridge Contract (v2 — Multi-Call Confirmation)
//
// Security model:
...
//   lichenid_address                      → [u8; 32]
//   lichenid_min_rep                      → u64
...
pub extern "C" fn initialize(owner_ptr: *const u8) -> u32 {
    ...
    log_info("LichenBridge v2 initialized");
    0
}
...
/// Lock LICN tokens for bridging to an external chain.
///
/// Parameters:
///   - sender_ptr: 32-byte sender address
///   - amount: number of spores to lock
...
pub extern "C" fn lock_tokens(
    ...
    if !check_identity_gate(&sender_arr) {
        log_info("Insufficient LichenID reputation for bridge");
        reentrancy_exit();
        return 10;
    }
...
///   - amount: number of spores to mint
...
pub extern "C" fn confirm_mint(caller_ptr: *const u8, nonce: u64) -> u32 {
    ...
    if (new_count as u64) >= required {
        ...
        recipient.copy_from_slice(&tx_data[0..32]);
...
///   - amount: number of spores to unlock
...
pub extern "C" fn set_token_address(caller_ptr: *const u8, addr_ptr: *const u8) -> u32 {
    ...
    if let Err(e) = require_owner(&caller) {
        log_info("Not owner");
        return e;
    }
...
mod tests {
    ...
    fn setup() {
        test_mock::reset();
        // AUDIT-FIX G11-01: Configure lichencoin + contract address for transfers
    ...
    fn test_identity_gate_blocks_lock_tokens() {
        ...
        test_mock::set_caller(owner);
        assert_eq!(
            set_lichenid_address(owner.as_ptr(), lichenid_addr.as_ptr()),
            0
        );
        test_mock::set_caller(owner);
        assert_eq!(set_identity_gate(owner.as_ptr(), 100), 0);

        // lock_tokens should be blocked (identity gate check)
        let sender = [3u8; 32];
        test_mock::set_caller(sender);
        test_mock::set_value(1_000_000);
        let result = lock_tokens(
    ...
    // =============================================
    // G11-01 NEW TESTS: Token transfer wiring
    // =============================================

    #[test]
    fn test_lock_insufficient_value() {
        setup();
        let owner = [1u8; 32];
        test_mock::set_caller(owner);
        initialize(owner.as_ptr());
    ...
    #[test]
    fn test_mint_without_licn_configured() {
        setup_no_licn();
        test_mock::SLOT.with(|s| *s.borrow_mut() = 200);
        let owner = [1u8; 32];
        test_mock::set_caller(owner);
        ...
        // Confirm triggers transfer_out which needs lichencoin address
        test_mock::set_caller(val2);
        assert_eq!(confirm_mint(val2.as_ptr(), 0), 30);
    }

    #[test]
    fn test_unlock_without_licn_configured() {
        setup_no_licn();
        test_mock::SLOT.with(|s| *s.borrow_mut() = 100);
        let owner = [1u8; 32];
        test_mock::set_caller(owner);
        ...
        test_mock::set_caller(owner);
        add_bridge_validator(owner.as_ptr(), val2.as_ptr());
        // Lock tokens (lock itself doesn't need licn)
        let sender = [5u8; 32];
        test_mock::set_caller(sender);
        test_mock::set_value(1_000_000);
        lock_tokens(
    ...
    fn test_set_token_address_happy() {
        ...
        let addr = [0x77u8; 32];
        test_mock::set_caller(owner);
        assert_eq!(set_token_address(owner.as_ptr(), addr.as_ptr()), 0);
        let stored = test_mock::get_storage(LICHENCOIN_ADDRESS_KEY).unwrap();
        assert_eq!(&stored[..32], &addr);
    }

    #[test]
    fn test_set_token_address_non_owner_rejected() {
        setup();
        let owner = [1u8; 32];
        test_mock::set_caller(owner);
        initialize(owner.as_ptr());
        let other = [9u8; 32];
        test_mock::set_caller(other);
        assert_eq!(set_token_address(other.as_ptr(), [0x77u8; 32].as_ptr()), 2);
    }
