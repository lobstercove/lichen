// LichenID - Agent Identity & Reputation System
...
const LICHENID_REENTRANCY_KEY: &[u8] = b"mid_reentrancy";

fn reentrancy_enter() -> bool {
    if let Some(v) = storage_get(LICHENID_REENTRANCY_KEY) {
        if !v.is_empty() && v[0] == 1 {
            return false;
        }
    }
    storage_set(LICHENID_REENTRANCY_KEY, &[1u8]);
    true
}

fn reentrancy_exit() {
    storage_set(LICHENID_REENTRANCY_KEY, &[0u8]);
}
...
const MIN_LICN_NAME_LEN: usize = 3;
...
fn is_premium_name(name: &[u8]) -> bool {
    name.len() >= MIN_LICN_NAME_LEN && name.len() <= PREMIUM_AUCTION_MAX_LEN
}
...
/// Validate a .lichen name: 3-32 chars, alphanumeric + hyphens, no leading/trailing hyphens, lowercase
fn validate_licn_name(name: &[u8]) -> bool {
    if name.len() < MIN_LICN_NAME_LEN || name.len() > MAX_LICN_NAME_LEN {
        return false;
    }
...
/// Initialize the LichenID program.
...
pub extern "C" fn initialize(admin_ptr: *const u8) -> u32 {
    ...
    log_info("🪪 Initializing LichenID program...");
    ...
    if storage_get(b"mid_admin").is_some() {
        log_info("LichenID already initialized");
        reentrancy_exit();
        return 1;
    }
...
pub extern "C" fn register_identity(
    ...
    log_info("🪪 Registering new LichenID identity...");

    if is_mid_paused() {
        log_info("LichenID is paused");
        reentrancy_exit();
        return 20;
    }
...
pub extern "C" fn update_reputation_typed(
    ...
    let admin = match storage_get(b"mid_admin") {
        Some(data) => data,
        None => {
            log_info("LichenID not initialized");
            reentrancy_exit();
            return 1;
        }
    };
...
pub extern "C" fn get_skills(pubkey_ptr: *const u8) -> u32 {
    ...
    lichen_sdk::set_return_data(&all_skills);
    0
}
...
pub extern "C" fn execute_recovery(
    ...
    for (old_key, new_key) in [
        ...
        if let Some(data) = storage_get(&old_key) {
            ...
            lichen_sdk::storage::remove(&old_key);
        }
    }
    ...
    if let Some(guardians) = storage_get(&old_gk) {
        ...
        lichen_sdk::storage::remove(&old_gk);
    }
...
pub extern "C" fn get_reputation(pubkey_ptr: *const u8) -> u32 {
    ...
    if let Some(mut record) = storage_get(&id_key) {
        ...
        lichen_sdk::set_return_data(&u64_to_bytes(rep));
        return 0;
    }
    ...
    if let Some(data) = storage_get(&rep_key) {
        if data.len() >= 8 {
            lichen_sdk::set_return_data(&data[0..8]);
            return 0;
        }
    }
...
pub extern "C" fn get_vouches(pubkey_ptr: *const u8) -> u32 {
    ...
    lichen_sdk::set_return_data(&all_vouches);
    0
}
...
const ACHIEVEMENT_NAME_REGISTRAR: u8 = 9; // Registered a .lichen name
const ACHIEVEMENT_SKILL_MASTER: u8 = 10; // Added 5+ skills
const ACHIEVEMENT_SOCIAL: u8 = 11; // Received 3+ vouches
const ACHIEVEMENT_FIRST_NAME: u8 = 12; // Registered first .lichen name
...
const _ACHIEVEMENT_MOSSSTAKE: u8 = 43;
...
const _ACHIEVEMENT_STLICN_TRANSFER: u8 = 48;
...
pub extern "C" fn revoke_attestation(
    ...
    // Remove attestation
    lichen_sdk::storage::remove(&active_ak);
...
///
/// Parameters:
///   - caller_ptr: 32-byte owner address (must have LichenID)
///   - name_ptr: pointer to name bytes (lowercase, no .lichen suffix)
///   - name_len: length of name
...
pub extern "C" fn register_name(
    ...
    log_info("Registering .lichen name...");
    ...
    // Must have a LichenID
    let id_key = identity_key(&caller);
    if storage_get(&id_key).is_none() {
        log_info("Must register LichenID first");
        reentrancy_exit();
        return 1;
    }

    // Validate name
    if !validate_licn_name(&name) {
        log_info("Invalid .lichen name (3-32 chars, a-z 0-9 hyphens, no leading/trailing hyphens)");
        reentrancy_exit();
        return 2;
    }
    ...
    if let Some(existing_name) = storage_get(&rev_key) {
        ...
        if let Some(nr) = storage_get(&existing_nk) {
            if nr.len() >= 48 {
                ...
                if lichen_sdk::get_slot() < expiry {
                    log_info("Already have a .lichen name; release it first");
                    reentrancy_exit();
                    return 6;
                }
            }
...
// ============================================================================
// REVERSE RESOLVE (address → .lichen name)
// ============================================================================

/// Reverse resolve: given an address, return its .lichen name.
...
pub extern "C" fn reverse_resolve(addr_ptr: *const u8) -> u32 {
    ...
    match storage_get(&rev_key) {
        Some(name_bytes) => {
            ...
            if let Some(record) = storage_get(&nk) {
                if record.len() >= 48 {
                    ...
                    if lichen_sdk::get_slot() < expiry {
                        lichen_sdk::set_return_data(&name_bytes);
                        return 0;
                    }
                }
            }
        ...
        None => {
            log_info("No .lichen name for this address");
            1
        }
    }
}
...
pub extern "C" fn create_name_auction(
    ...
    if !validate_licn_name(&name) {
        log_info("Invalid .lichen name for auction");
        reentrancy_exit();
        return 2;
    }
    ...
    let now_slot = lichen_sdk::get_slot();
...
pub extern "C" fn bid_name_auction(
    ...
    if storage_get(&identity_key(&bidder)).is_none() {
        log_info("Bidder must have a LichenID");
        reentrancy_exit();
        return 1;
    }
    ...
    if let Some(existing_name) = storage_get(&winner_rev) {
        ...
        if let Some(nr) = storage_get(&existing_nk) {
            if nr.len() >= 48 {
                let expiry = bytes_to_u64(&nr[40..48]);
                if now_slot < expiry {
                    log_info("Winner already has an active .lichen name");
                    reentrancy_exit();
                    return 7;
                }
            }
...
/// The new owner must have a LichenID and must not already own a name.
///
/// Parameters:
...
pub extern "C" fn transfer_name(
    ...
    if let Some(existing_name) = storage_get(&new_rev) {
        ...
        if let Some(nr) = storage_get(&existing_nk) {
            if nr.len() >= 48 {
                ...
                if lichen_sdk::get_slot() < ex {
                    log_info("New owner already has a .lichen name");
                    reentrancy_exit();
                    return 5;
                }
            }
    ...
    log_info(".lichen name transferred");
    reentrancy_exit();
    0
}
...
pub extern "C" fn release_name(caller_ptr: *const u8, name_ptr: *const u8, name_len: u32) -> u32 {
    ...
    lichen_sdk::storage::remove(&nk);

    // Remove reverse mapping
    let rev_key = name_reverse_key(&caller);
    lichen_sdk::storage::remove(&rev_key);
    ...
    log_info(".lichen name released");
    reentrancy_exit();
    0
}

/// Delegated transfer of a .lichen name.
#[no_mangle]
pub extern "C" fn transfer_name_as(
    ...
    log_info("Delegated .lichen name transferred");
    reentrancy_exit();
    0
}

/// Delegated renewal of a .lichen name.
#[no_mangle]
pub extern "C" fn renew_name_as(
    ...
    log_info("Delegated .lichen name renewed");
    reentrancy_exit();
    0
}

/// Delegated release of a .lichen name.
#[no_mangle]
pub extern "C" fn release_name_as(
    ...
    log_info("Delegated .lichen name released");
    reentrancy_exit();
    0
}
...
/// Set service rate (LICN per unit).
///
/// Parameters:
///   - caller_ptr: 32-byte owner address
///   - licn_per_unit: rate in LICN
#[no_mangle]
pub extern "C" fn set_rate(caller_ptr: *const u8, licn_per_unit: u64) -> u32 {
    if !reentrancy_enter() {
        return 100;
    }
    let mut caller = [0u8; 32];
    ...
    storage_set(&rk, &u64_to_bytes(licn_per_unit));
...
pub extern "C" fn set_rate_as(
    ...
    storage_set(&rate_key(&owner), &u64_to_bytes(licn_per_unit));
    ...
    match storage_get(&rev_key) {
        Some(name_bytes) => {
            ...
            let valid = if let Some(nr) = storage_get(&nk) {
                if nr.len() >= 48 {
                    lichen_sdk::get_slot() < bytes_to_u64(&nr[40..48])
                } else {
                    false
                }
            } else {
...
pub extern "C" fn mid_pause(caller_ptr: *const u8) -> u32 {
    ...
    log_info("LichenID paused");
    reentrancy_exit();
    0
}

/// Unpause LichenID. Admin only.
...
mod tests {
    ...
    fn test_get_agent_profile() {
        ...
        // Register name
        let licn_name = b"profiled";
        register_name(
            owner.as_ptr(),
            licn_name.as_ptr(),
            licn_name.len() as u32,
            1,
        );
        ...
        assert_eq!(nm_len, licn_name.len());
        assert_eq!(
    ...
    fn test_reentrancy_guard_blocks_recursive_call() {
        ...
        assert_eq!(bid_name_auction(addr.as_ptr(), name.as_ptr(), 4, 100), 1); // "Bidder must have LichenID"
    }
