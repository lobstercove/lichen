// wNEO Token — Wrapped NEO on Lichen
//
// Architecture:
//   wNEO is a 1:1 receipt token backed by NEO reserves held in the
//   Lichen treasury (Neo X or Neo N3 wallet). Users deposit NEO on Neo X or Neo N3,
//   custody service sweeps to treasury, then mints wNEO on Lichen.
//
...
//
// DEX Integration:
//   wNEO/lUSD — NEO priced in USD
//   wNEO/LICN — NEO priced in LICN (direct, no stablecoin needed)
...
#[allow(dead_code)]
const TOKEN_NAME: &[u8] = b"Wrapped NEO";
#[allow(dead_code)]
const TOKEN_SYMBOL: &[u8] = b"wNEO";
#[allow(dead_code)]
const DECIMALS: u8 = 9; // 9-decimal precision for wrapped NEO

// Minting controls
const MINT_CAP_PER_EPOCH: u64 = 100_000_000_000_000; // 100K wNEO per epoch — circuit breaker, not growth limiter
...
const ERR_WHOLE_LOT_REQUIRED: u32 = 14;
const WHOLE_NEO_LOT: u64 = 1_000_000_000;

// Storage keys — prefixed "wneo_" to avoid collision
const ADMIN_KEY: &[u8] = b"wneo_admin";
const PENDING_ADMIN_KEY: &[u8] = b"wneo_pending_admin";
const ATTESTER_KEY: &[u8] = b"wneo_attester";
const MINTER_KEY: &[u8] = b"wneo_minter";
const BOOTSTRAP_COMPLETE_KEY: &[u8] = b"wneo_bootstrap_complete";
const PAUSED_KEY: &[u8] = b"wneo_paused";
const REENTRANCY_KEY: &[u8] = b"wneo_reentrancy";
const TOTAL_SUPPLY_KEY: &[u8] = b"wneo_supply";
const TOTAL_MINTED_KEY: &[u8] = b"wneo_minted";
const TOTAL_BURNED_KEY: &[u8] = b"wneo_burned";

const RESERVE_ATTESTED_KEY: &[u8] = b"wneo_reserve_att";
const RESERVE_SLOT_KEY: &[u8] = b"wneo_reserve_slot";
const RESERVE_HASH_KEY: &[u8] = b"wneo_reserve_hash";
const ATTESTATION_COUNT_KEY: &[u8] = b"wneo_att_count";

const EPOCH_START_KEY: &[u8] = b"wneo_epoch_start";
const EPOCH_MINTED_KEY: &[u8] = b"wneo_epoch_mint";

const MINT_EVENT_COUNT_KEY: &[u8] = b"wneo_mint_evt";
const BURN_EVENT_COUNT_KEY: &[u8] = b"wneo_burn_evt";
const TRANSFER_COUNT_KEY: &[u8] = b"wneo_xfer_cnt";
...
fn is_whole_neo_lot(amount: u64) -> bool {
    amount % WHOLE_NEO_LOT == 0
}
...
fn attestation_key(index: u64) -> Vec<u8> {
    let mut k = Vec::from(&b"wneo_att_"[..]);
...
pub extern "C" fn initialize(admin: *const u8) -> u32 {
    ...
    log_info("wNEO token initialized");
    0
}
...
pub extern "C" fn mint(caller: *const u8, to: *const u8, amount: u64) -> u32 {
    ...
    if !is_whole_neo_lot(amount) {
        reentrancy_exit();
        return ERR_WHOLE_LOT_REQUIRED;
    }

    if let Err(code) = check_reserve_circuit_breaker(amount) {
        reentrancy_exit();
        if code == ERR_CIRCUIT_BREAKER {
            log_info("CIRCUIT BREAKER: wNEO mint blocked - exceeds attested reserves");
        }
        return code;
    }
    ...
    let (next_epoch_start, next_epoch_minted) = match next_epoch_state(amount) {
        ...
        Err(code) => {
            ...
            if code == ERR_EPOCH_CAP {
                log_info("RATE LIMIT: wNEO epoch mint cap reached");
            }
            return code;
        }
    };
...
pub extern "C" fn burn(caller: *const u8, amount: u64) -> u32 {
    ...
    let mut msg = Vec::from(&b"BURN wNEO #"[..]);
    msg.extend_from_slice(&u64_to_decimal(next_evt_count));
    msg.extend_from_slice(b": ");
    msg.extend_from_slice(&u64_to_decimal(amount));
    msg.extend_from_slice(b" fixed units from 0x");
...
pub extern "C" fn attest_reserves(
    ...
    if !is_whole_neo_lot(reserve_amount) {
        return ERR_WHOLE_LOT_REQUIRED;
    }
    ...
    let mut msg = Vec::from(&b"wNEO RESERVE ATTESTATION #"[..]);
    msg.extend_from_slice(&u64_to_decimal(next_count));
    msg.extend_from_slice(b": ");
    msg.extend_from_slice(&u64_to_decimal(reserve_amount));
    msg.extend_from_slice(b" fixed units backing declared");
    log_info(core::str::from_utf8(&msg).unwrap_or("event"));
...
pub extern "C" fn transfer_admin(caller: *const u8, new_admin: *const u8) -> u32 {
    ...
    log_info("wNEO: pending admin set");
    0
}
...
pub extern "C" fn set_minter(caller: *const u8, new_minter: *const u8) -> u32 {
    ...
    log_info("wNEO: mint authority updated");
    0
}
...
pub extern "C" fn set_attester(caller: *const u8, new_attester: *const u8) -> u32 {
    ...
    log_info("wNEO: reserve attester updated");
    0
}
...
pub extern "C" fn complete_bootstrap(caller: *const u8) -> u32 {
    ...
    log_info("wNEO: bootstrap completed");
    0
}
...
mod tests {
    ...
    fn test_mint_and_burn() {
        ...
        assert_eq!(mint(admin.as_ptr(), user.as_ptr(), 2_000_000_000), 0);
        assert_eq!(balance_of(user.as_ptr()), 2_000_000_000);
        assert_eq!(total_supply(), 2_000_000_000);

        test_mock::set_caller(user);
        assert_eq!(burn(user.as_ptr(), 1_000_000_000), 0);
        assert_eq!(balance_of(user.as_ptr()), 1_000_000_000);
        assert_eq!(total_supply(), 1_000_000_000);
        assert_eq!(total_burned(), 1_000_000_000);
    }

    #[test]
    fn test_whole_lot_enforcement() {
        test_mock::reset();
        let admin = addr(1);
        let user = addr(2);
        let bob = addr(3);
        ...
        test_mock::set_caller(admin);
        assert_eq!(
            mint(admin.as_ptr(), user.as_ptr(), 1_500_000_000),
            ERR_WHOLE_LOT_REQUIRED
        );
        assert_eq!(total_supply(), 0);
        assert_eq!(
            attest_reserves(admin.as_ptr(), 1_500_000_000, proof.as_ptr()),
            ERR_WHOLE_LOT_REQUIRED
        );
        assert_eq!(get_attestation_count(), 0);
        assert_eq!(mint(admin.as_ptr(), user.as_ptr(), 2_000_000_000), 0);

        test_mock::set_caller(user);
        assert_eq!(
            transfer(user.as_ptr(), bob.as_ptr(), 500_000_000),
            ERR_WHOLE_LOT_REQUIRED
        );
        assert_eq!(
            approve(user.as_ptr(), dex.as_ptr(), 500_000_000),
            ERR_WHOLE_LOT_REQUIRED
        );
        assert_eq!(burn(user.as_ptr(), 500_000_000), ERR_WHOLE_LOT_REQUIRED);
        assert_eq!(balance_of(user.as_ptr()), 2_000_000_000);

        assert_eq!(approve(user.as_ptr(), dex.as_ptr(), 1_000_000_000), 0);
        test_mock::set_caller(dex);
        assert_eq!(
            transfer_from(dex.as_ptr(), user.as_ptr(), bob.as_ptr(), 500_000_000),
            ERR_WHOLE_LOT_REQUIRED
        );
        assert_eq!(balance_of(user.as_ptr()), 2_000_000_000);
        assert_eq!(allowance(user.as_ptr(), dex.as_ptr()), 1_000_000_000);
    }
    ...
    fn test_reserve_circuit_breaker() {
        ...
        assert_eq!(mint(admin.as_ptr(), user.as_ptr(), 1_000_000_000), 10); // blocked
    }
    ...
    fn test_burn_underflow_preserves_state() {
        ...
        let user = addr(2);
        initialize(admin.as_ptr());
        save_u64(&balance_key(&user), 1_000_000_000);
        save_u64(TOTAL_SUPPLY_KEY, 0);
        save_u64(TOTAL_BURNED_KEY, 9);

        test_mock::set_caller(user);
        assert_eq!(burn(user.as_ptr(), 1_000_000_000), ERR_ARITHMETIC_OVERFLOW);
        assert_eq!(balance_of(user.as_ptr()), 1_000_000_000);
        assert_eq!(total_supply(), 0);
    ...
    fn test_transfer_overflow_preserves_state() {
        ...
        let bob = addr(3);
        initialize(admin.as_ptr());
        save_u64(&balance_key(&alice), 1_000_000_000);
        save_u64(&balance_key(&bob), u64::MAX);

        test_mock::set_caller(alice);
        assert_eq!(
            transfer(alice.as_ptr(), bob.as_ptr(), 1_000_000_000),
            ERR_ARITHMETIC_OVERFLOW
        );
        assert_eq!(balance_of(alice.as_ptr()), 1_000_000_000);
    ...
    fn test_pause_blocks_operations() {
        ...
        initialize(admin.as_ptr());
        test_mock::set_caller(admin);
        mint(admin.as_ptr(), user.as_ptr(), 2_000_000_000);
        emergency_pause(admin.as_ptr());
        test_mock::set_caller(user);
        assert_eq!(transfer(user.as_ptr(), admin.as_ptr(), 1_000_000_000), 1);
        assert_eq!(burn(user.as_ptr(), 1_000_000_000), 0);
        test_mock::set_caller(admin);
        emergency_unpause(admin.as_ptr());
        test_mock::set_caller(user);
        assert_eq!(transfer(user.as_ptr(), admin.as_ptr(), 1_000_000_000), 0);
    }
