// lUSD Token — Treasury-Backed Stable Unit for Lichen DEX
//
// Architecture:
//   lUSD is a 1:1 receipt token backed by USDT/USDC reserves held in the
//   Lichen treasury. It is NOT an algorithmic stablecoin — it's a
...
//   - Total minted lUSD is tracked on-chain (auditable at any time)
//   - reserve_attestation() records periodic reserve proofs linked to
//     external auditor reports or LichenOracle price feeds
...
//             → treasury calls mint(to, amount) → user receives lUSD on Lichen
//   Withdraw: User calls burn(amount) → treasury releases USDT or USDC
//             to user's preferred address/network
//
// DEX Integration:
//   All trading pairs: LICN/lUSD, BTC/lUSD, ETH/lUSD, MOSS/lUSD
//   All AMM pools: paired against lUSD
//   All fee collection: denominated in lUSD
//   All margin collateral: held in lUSD
//
// Security:
...
// Token metadata
#[allow(dead_code)]
const TOKEN_NAME: &[u8] = b"Lichen USD";
#[allow(dead_code)]
const TOKEN_SYMBOL: &[u8] = b"lUSD";
#[allow(dead_code)]
...
pub extern "C" fn initialize(admin: *const u8) -> u32 {
    ...
    log_info("lUSD token initialized");
    0
}

/// Mint new lUSD. Only callable by admin (treasury multisig).
...
/// Burn lUSD. Called by the holder or by admin on behalf of a withdrawal.
...
/// Transfer lUSD between accounts. Standard token transfer.
#[no_mangle]
...
/// proof_hash: 32-byte hash of the external audit report or LichenOracle feed.
#[no_mangle]
...
/// Get lUSD balance of an account.
#[no_mangle]
...
mod tests {
    ...
    fn test_compliance_blocks_burn_without_mutation() {
        ...
        test_mock::set_caller(user);
        assert_eq!(burn(user.as_ptr(), 2_000_000), ERR_COMPLIANCE_RESTRICTED);
        assert_eq!(balance_of(user.as_ptr()), 5_000_000);
        assert_eq!(total_supply(), 5_000_000);
    ...
    fn test_compliance_blocks_transfer_without_mutation() {
        ...
        initialize(admin.as_ptr());
        test_mock::set_caller(admin);
        assert_eq!(mint(admin.as_ptr(), alice.as_ptr(), 10_000_000), 0);

        test_mock::set_can_transfer(false);
        test_mock::set_caller(alice);
        assert_eq!(
            transfer(alice.as_ptr(), bob.as_ptr(), 3_000_000),
            ERR_COMPLIANCE_RESTRICTED
        );
        assert_eq!(balance_of(alice.as_ptr()), 10_000_000);
