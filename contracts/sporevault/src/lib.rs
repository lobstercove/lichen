// SporeVault v2 - Yield Aggregator
        ...
        let strategy_yield = match strategy_type {
            STRATEGY_LENDING => {
                match query_protocol_yield(
                    ...
                    None => {
                        log_info(
                            "harvest: ThallLend yield quote unavailable — skipping lending yield",
                        );
            ...
            STRATEGY_LP => {
                ...
                log_info(
                    "harvest: LichenSwap LP yield is embedded in pool share value; no standalone reward export",
                );
                unavailable_yield_surfaces += 1;
                0
            }
...
mod tests {
    ...
    fn test_set_protocol_addresses_success() {
        ...
        let stored_ll = test_mock::get_storage(THALLLEND_ADDRESS_KEY).unwrap();
        assert_eq!(stored_ll.as_slice(), &thalllend);
        let stored_ms = test_mock::get_storage(LICHENSWAP_ADDRESS_KEY).unwrap();
        assert_eq!(stored_ms.as_slice(), &lichenswap);
    }
    ...
    fn test_harvest_with_protocol_addresses_configured() {
        ...
        // Only the lending strategy yields because LichenSwap exposes no standalone LP reward export.
        let total_assets = load_u64(b"cv_total_assets");
