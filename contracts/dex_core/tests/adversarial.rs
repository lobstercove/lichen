...
fn test_place_order_quantity_exceeds_max() {
    ...
    lichen_sdk::test_mock::set_slot(100);
    lichen_sdk::test_mock::set_caller(trader);
    // MAX_ORDER_SIZE is 10_000_000_000_000_000 (10M LICN at 9 decimals)
...
fn test_fee_treasury_accumulation() {
    ...
    lichen_sdk::test_mock::set_caller(seller);
    assert_eq!(
        place_order(seller.as_ptr(), pair_id, 1, 0, P, big_q, 0, 0),
        0
    );
    lichen_sdk::test_mock::set_caller(buyer);
    assert_eq!(
...
fn test_expired_maker_skipped_during_matching() {
    ...
    lichen_sdk::test_mock::set_slot(201);
    lichen_sdk::test_mock::set_caller(buyer);
...
fn test_unpause_restores_operations() {
    ...
    lichen_sdk::test_mock::set_slot(100 + UNPAUSE_TIMELOCK_SLOTS);
...
fn test_cancel_someone_elses_order() {
    ...
    let trader_b = [3u8; 32];
    lichen_sdk::test_mock::set_slot(100);
    lichen_sdk::test_mock::set_caller(trader_a);
    assert_eq!(place_order(trader_a.as_ptr(), pair_id, 0, 0, P, Q, 0, 0), 0);
    lichen_sdk::test_mock::set_caller(trader_b);
