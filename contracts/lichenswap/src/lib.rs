// LichenSwap - Automated Market Maker DEX (v2 — DEEP hardened)
...
const LICHENID_DISCOUNT_THRESHOLD_KEY: &[u8] = b"lichenid_disc_threshold";
/// Storage key for reputation discount in basis points
const LICHENID_DISCOUNT_BPS_KEY: &[u8] = b"lichenid_disc_bps";
...
pub extern "C" fn set_lichenid_address(caller_ptr: *const u8, lichenid_addr_ptr: *const u8) -> u32 {
    ...
    let real_caller = get_caller();
    if real_caller.0 != caller {
        log_info("set_lichenid_address rejected: caller mismatch");
        return 1;
    }
...
fn get_reputation_bonus(amount_out: u64) -> u64 {
    let threshold = match storage_get(LICHENID_DISCOUNT_THRESHOLD_KEY) {
    ...
    let discount_bps = match storage_get(LICHENID_DISCOUNT_BPS_KEY) {
        Some(data) if data.len() >= 8 => bytes_to_u64(&data),
        _ => return 0,
    };
    if discount_bps == 0 {
...
mod tests {
    ...
    fn test_set_reputation_discount_admin_only() {
        ...
        let threshold = test_mock::get_storage(LICHENID_DISCOUNT_THRESHOLD_KEY).unwrap();
        assert_eq!(bytes_to_u64(&threshold), 100);
        let bps = test_mock::get_storage(LICHENID_DISCOUNT_BPS_KEY).unwrap();
    ...
    #[test]
    fn test_set_lichenid_address_wrong_caller() {
        setup();
        let admin = [1u8; 32];
        test_mock::set_caller(admin);
        assert_eq!(set_identity_admin(admin.as_ptr()), 0);

        // Non-admin tries to set LichenID address with mismatched caller
        let attacker = [9u8; 32];
        let lichenid = [0x42u8; 32];
        test_mock::set_caller(attacker);
        let result = set_lichenid_address(attacker.as_ptr(), lichenid.as_ptr());
