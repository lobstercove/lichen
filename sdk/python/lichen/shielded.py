...
variable-length Plonky3 STARK proof bytes.  In production the proofs are
generated by the Lichen Plonky3 prover (lichen-core::zk::Prover).
"""
...
def shield_instruction(
    ...
    Deposits ``amount`` spores from ``sender`` into the shielded pool,
...
def unshield_instruction(
    ...
    Withdraws ``amount`` spores from the shielded pool to ``recipient``.
