...
assert(
  productionDeployment.includes('Current signed-release target for this runbook is `v0.5.193`') &&
    productionDeployment.includes('keep `v0.5.192` as the signed rollback point') &&
    ...
    productionDeployment.includes('v0.5.193` keeps the `v0.5.192` safe resume checkpoint repair baseline and adds deterministic MossStake repair snapshot bytes') &&
    ...
    productionDeployment.includes('export LICHEN_RELEASE_TAG=v0.5.193') &&
  ...
  'production clean-slate checklist must match current v0.5.193 rollback v0.5.192/32-symbol/13-market expectations',
);
assert(
