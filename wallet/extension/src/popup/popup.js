...
function getPopupBalanceSnapshot(result) {
  const spendable = spendableBaseUnitsPopup(result);
  return {
...
async function exportKeystoreJsonAction() {
  ...
  try {
    const password = getSettingsPassword();
    const privateKeyHex = await decryptPrivateKey(wallet.encryptedKey, password);
    const encryptedSeed = await encryptPrivateKey(privateKeyHex, password);
...
async function loadAssets() {
  ...
  try {
    ...
    const spendableRaw = Number(spendableBaseUnitsPopup(result));
  ...
  try {
    ...
    const html = txs.map((tx) => {
      ...
      if (tx.type === 'Shield') {
        ...
        if (tx.type === 'MossStakeDeposit' || tx.type === 'MossStakeUnstake' || tx.type === 'Stake') sign = '-';
        else if (tx.type === 'MossStakeClaim') sign = '+';
    ...
    if (licnName) {
      try {
        ...
        if (nameDetails && nameDetails.expiry_slot && currentSlot > 0) {
          ...
          const remainingSlots = nameDetails.expiry_slot - currentSlot;
          if (remainingSlots <= 0) {
            nameExpiryHtml = '<div style="font-size:0.72rem;color:#ef4444;margin-top:0.25rem;"><i class="fas fa-exclamation-circle"></i> Domain expired</div>';
          } else {
            ...
            const daysLabel = remainingDays > 365
              ...
              : `${remainingDays}d`;
            const color = remainingDays <= 90 ? '#f59e0b' : 'var(--text-muted)';
            nameExpiryHtml = `<div style="font-size:0.72rem;color:${color};margin-top:0.25rem;"><i class="fas fa-clock"></i> Expires ${dateStr} (${daysLabel} left)</div>`;
          }
        }
      } catch (_) { /* name detail fetch is best-effort */ }
    }
...
async function loadExtensionStaking() {
  ...
  try {
    const [position, poolInfo, queue, balance] = await Promise.all([
      ...
      rpc.getBalance(wallet.address).catch(() => null),
    ]);
    ...
    const spendableSpores = balance ? spendableBaseUnitsPopup(balance) : null;
...
async function refreshBalance() {
  ...
  try {
    ...
    window._cachedSpendableSpores = spendableBaseUnitsPopup(result).toString();
    ...
    if (avail) avail.textContent = `Available: ${balanceSnapshot.spendableLicn.toLocaleString(undefined, { maximumFractionDigits: 9 })} LICN`;

    // Disable Send button when balance can't cover fee
    const sendBtn = document.getElementById('sendNow');
    if (sendBtn) {
      if (balanceSnapshot.spendableLicn <= 0.001) {
...
async function handleSendNow() {
  ...
  try {
    ...
    const spendable = spendableBaseUnitsPopup(balResult);
    const feeSpores = 1_000_000n;
...
async function saveRpcSettings() {
  ...
  setStatus('RPC/custody endpoints saved. Bridge routing and contract metadata stay pinned to trusted endpoints.');
}
...
async function parseKeystoreToPrivateKeyHex(rawJson, password) {
  ...
  if (data?.encryptedSeed) {
    return normalizePrivateKeyHex(await decryptPrivateKey(data.encryptedSeed, password));
  }

  if (typeof data?.seed === 'string') {
    return normalizePrivateKeyHex(data.seed);
  }

  if (Array.isArray(data?.seed)) {
    const bytes = Uint8Array.from(data.seed);
    if (bytes.length !== 32) {
      throw new Error('seed array must be 32 bytes');
    }
  ...
  if (Array.isArray(data?.privateKey)) {
    ...
    if (bytes.length !== 32) {
      throw new Error('privateKey array must be 32 bytes');
    }
    return bytesToHex(bytes);
  }
...
async function handleImportSave() {
  ...
  if (importType === 'json') {
    try {
      const privateKeyHex = await parseKeystoreToPrivateKeyHex(keystoreRaw, password);
  ...
  if (typeof QRCode === 'undefined') {
    container.innerHTML = `
      <div style="padding:1rem;border:1px dashed var(--border);border-radius:12px;text-align:center;color:var(--text-muted);">
        ...
        <div style="font-size:0.78rem;">QR preview unavailable in the popup</div>
        <div style="font-size:0.72rem;margin-top:0.2rem;">Use Copy or Open Full to receive</div>
      </div>`;
    return;
  }
  try {
...
async function deriveShieldedSeedFromWallet(wallet, password) {
  ...
  try {
    ...
    const seedBytes = hexToBytesLocal(decryptedSeedHex);
    ...
    zeroBytesLocal(seedBytes);
    zeroBytesLocal(keyMaterial);
    return shieldSeed;
  } finally {
...
async function initializeShieldedPopupForActiveWallet() {
  const wallet = getActiveWallet();
  if (!wallet) return false;
  if (shieldedPopupState.initialized) return true;

  const password = await securePasswordPrompt('Enter your wallet password to initialize shielded privacy.');
  if (!password) {
    setStatus('Shielded initialization cancelled');
    return false;
  }

  let shieldSeed = null;
  try {
    shieldSeed = await deriveShieldedSeedFromWallet(wallet, password);
    if (!shieldSeed) {
      setStatus('Shielded initialization unavailable');
      return false;
    }
    ...
    alert(`Shielded initialization failed: ${error?.message || error}`);
    return false;
  } finally {
    zeroBytesLocal(shieldSeed);
  }
}
...
async function loadShieldPanel() {
  ...
  if (addrEl) {
    addrEl.value = shieldedPopupState.shieldedAddress || 'Initialize shielded wallet to derive';
  }
  const vkEl = document.getElementById('extViewingKey');
  if (vkEl) {
    vkEl.value = shieldedPopupState.viewingKey ? bytesToHexLocal(shieldedPopupState.viewingKey) : 'Initialize shielded wallet to reveal';
  }
...
async function requestExtBridgeDeposit(chain, asset, chainName) {
  ...
  try {
    ...
    // Poll deposit status
    startExtDepositPolling(data.deposit_id, wallet);
...
function startExtDepositPolling(depositId, wallet) {
  ...
  extDepositPollInterval = setInterval(async () => {
    try {
      ...
      if (String(error?.message || '').includes('Bridge authorization expired')) {
        const statusEl = document.getElementById('extDepositStatus');
        if (statusEl) {
          statusEl.innerHTML = '<i class="fas fa-lock" style="color:#EF476F;"></i> Bridge authorization expired. Start a new deposit flow to refresh access.';
        }
