...
async function updateDashboardStats() {
    ...
    try {
        ...
        if (rewardInfo) {
            ...
            if (supplyChangeEl) {
                supplyChangeEl.textContent = 'Genesis + minted - burned';
            }
...
function getTransactionPillClass(type) {
    ...
    const mapped = {
        ...
        unstake: 'unstake',
        system: 'system',
        registervalidator: 'system',
        slashvalidator: 'system',
    };
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const WS_STALE_THRESHOLD = 30000;
    ...
    const startStaleCheck = () => {
        ...
        staleCheckInterval = setInterval(() => {
            if (typeof ws !== 'undefined' && ws.isConnected()) {
                const activityAt = Math.max(lastWsBlockTime, ws.getLastActivityAt?.() || 0);
                if (!activityAt) {
                    return;
                }
                ...
                if (elapsed > WS_STALE_THRESHOLD) {
                    staleStrikeCount += 1;
                    console.warn(`WebSocket idle for ${Math.round(elapsed / 1000)}s — refreshing dashboard (${staleStrikeCount}/2)`);
                    updateDashboardStats();
                    updateLatestTransactions();
                    updateLatestBlocks();

                    if (staleStrikeCount >= 2) {
                        console.warn(`WebSocket reconnect after ${Math.round(elapsed / 1000)}s without activity`);
                        staleStrikeCount = 0;
    ...
    let navPanel = navContainer?.querySelector('.nav-mobile-panel');
    const mobileNavQuery = window.matchMedia('(max-width: 768px)');

    if (navToggle && navMenu) {
        const closeNav = () => {
            navMenu.classList.remove('active', 'open');
            navActions?.classList.remove('active', 'open');
            navPanel?.classList.remove('active', 'open');
            navToggle.classList.remove('active');
            navContainer?.style.setProperty('--nav-menu-height', '0px');
        };

        const mountMobileNav = () => {
            if (!navContainer || !navActions || !mobileNavQuery.matches) {
                return;
            }

            if (!navPanel) {
                navPanel = document.createElement('div');
                navPanel.className = 'nav-mobile-panel';
                navContainer.insertBefore(navPanel, navToggle || null);
            }

            if (navMenu.parentElement !== navPanel) {
                navPanel.appendChild(navMenu);
            }

            if (navActions.parentElement !== navPanel) {
                navPanel.appendChild(navActions);
            }
        };

        const unmountMobileNav = () => {
            closeNav();
            ...
            if (navPanel && navMenu.parentElement === navPanel) {
                navContainer.insertBefore(navMenu, navToggle || null);
            }

            if (navPanel && navActions?.parentElement === navPanel) {
                navContainer.insertBefore(navActions, navToggle || null);
            }

            if (navPanel?.parentElement) {
                navPanel.remove();
            }
        ...
        if (typeof mobileNavQuery.addEventListener === 'function') {
            mobileNavQuery.addEventListener('change', syncNavLayout);
        } else if (typeof mobileNavQuery.addListener === 'function') {
            mobileNavQuery.addListener(syncNavLayout);
        }
    }
});
