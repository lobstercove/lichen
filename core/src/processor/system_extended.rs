...
impl TxProcessor {
    ...
    pub(super) fn system_mossstake_unstake(&self, ix: &Instruction) -> Result<(), String> {
        ...
        if self.uses_legacy_wall_clock_mossstake() {
            let (current_slot, current_unix_seconds) = self.b_get_last_block_timestamp()?;
            pool.backfill_wall_clock_times(|slot| self.block_timestamp_for_slot(slot));
            let _request = pool.request_unstake_at_time(
                user,
                st_licn_amount,
                current_slot,
                current_unix_seconds,
    ...
    pub(super) fn system_mossstake_claim(&self, ix: &Instruction) -> Result<(), String> {
        ...
        let mut pool = self.b_get_mossstake_pool()?;
        let licn_claimed = if self.uses_legacy_wall_clock_mossstake() {
            let (current_slot, current_unix_seconds) = self.b_get_last_block_timestamp()?;
            pool.backfill_wall_clock_times(|slot| self.block_timestamp_for_slot(slot));
            pool.claim_unstake_at_time(user, current_slot, current_unix_seconds)?
        } else {
            let current_slot = self.b_get_last_slot().unwrap_or(0);
            pool.claim_unstake(user, current_slot)?
        };
