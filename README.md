...
**Current release:** signed `v0.5.123` with first-class BTC/wBTC support, slot-only MossStake state, deterministic legacy testnet bootstrap-grant replay, stale-checkpoint repair protection after replay, initial sync descendant catch-up, four-validator public testnet validation, and the v0.5.93 stable validator base without replaying historical state.
