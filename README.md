...
**Current release:** signed `v0.5.120` with first-class BTC/wBTC support, slot-only MossStake state, deterministic legacy testnet bootstrap-grant replay, fresh-join warp snapshot backpressure, live BFT restart catch-up recovery, guarded same-tip stake-pool singleton recovery, four-validator public testnet validation, and the v0.5.93 stable validator base without replaying historical state.
