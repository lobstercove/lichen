...
**Current release candidate:** `v0.5.206` with `v0.5.204` retained as the signed rollback point. It adds the guarded June 2026 testnet governed-signer recovery path and mainnet custody verification gates while keeping archive-backed public RPC mandatory outside dev mode, consensus-state-only repair snapshots for fresh/resumed validator repair, real public account/block/transaction history through the guarded read-only-source public-history merge path, and the guarded index-only public-history merge for source-backed Activity repairs when block bodies or slot cursors conflict but transaction/account indexes are clean. It preserves backed wallet and explorer account activity without replacing balances, validator state, contract storage, tip cursors, or other consensus data, aggregates homogeneous batched unshield transactions for wallet/explorer list summaries, rebuilds account transaction counters from existing backed rows before new block deltas, keeps full/fresh archive snapshot imports from merging stale target history, requests a full block immediately when compact-block reconstruction misses referenced transactions, restores mature non-genesis validators as active after restart once their persisted stake warmup has elapsed, keeps resumed validators in proposer rotation while only within the configured live BFT catch-up gap, lets restarted near-tip validators leave the pre-consensus sync gate once they are within a bounded voting-ready tolerance of the moving live tip, fixes successful LiveSync catch-up cooldown reset so restarted near-tip validators can immediately request the next small live gap, keeps the 400ms-slot BFT view-change timers that prevent offline proposer turns from dragging the chain into multi-second stalls, keeps the four-validator proposer-rotation fix, exact legacy testnet MossStake replay boundaries, canonical block-archive checkpoint snapshots with sorted commit certificates, canonical ledger snapshot exports derived from slot mappings instead of raw noncanonical hot RocksDB history, authenticated PQ checkpoint sources, staged checkpoint snapshot imports, shielded warp-snapshot state, first-class BTC/wBTC support, deterministic legacy bootstrap-grant replay, signed bootstrap-recovery reclassification, recent canonical post-block effects recovery for stake-pool singleton convergence, and the v0.5.93 stable validator base without replaying historical state incorrectly.
...
Examples:
- `https://github.com/lobstercove/lichen/releases/download/v0.5.206/lichen-validator-linux-x86_64.tar.gz`
- `https://github.com/lobstercove/lichen/releases/download/v0.5.206/lichen-validator-darwin-aarch64.tar.gz`
- `https://github.com/lobstercove/lichen/releases/download/v0.5.206/lichen-validator-windows-x86_64.tar.gz`
