...
**Current release candidate:** `v0.5.187` with `v0.5.186` retained as the signed rollback point. It keeps exact legacy testnet MossStake replay boundaries: pre-v0.5.93 history replays slot-only, the retained v0.5.93 through v0.5.97 interval replays wall-clock MossStake state, and v0.5.98+ cleanup resumes slot-only state. It also includes canonical block-archive checkpoint snapshots with sorted commit certificates, canonical ledger snapshot exports derived from slot mappings instead of raw noncanonical hot RocksDB history, canonical duplicate-tip pending descendant drain for resumed catch-up, recent verified checkpoint anchor advertisement for common snapshot quorum discovery, root-only checkpoint metadata quorum before heavy snapshot manifest scans, authenticated PQ node checkpoint sources for clean far-behind joiners, exact checkpoint snapshot slot/root/manifest anchors, deterministic archive manifest verification before live snapshot commit, received-chunk manifest verification before normalized staging re-exports, state-repair checkpoint manifests that avoid full archival scans during fresh or resumed consensus repair, restored pending-block overlap for initial sync replay recovery, bounded block-range replay while warp checkpoint metadata is probed, then paused block replay while a verified checkpoint snapshot transfer is active, first-class BTC/wBTC support, full hot column-family checkpoint snapshots, streamed checkpoint snapshot imports through a verified staging DB, partial-archive checkpoint source rejection, pinned checkpoint repair sources, bounded checkpoint-metadata probes, background verified checkpoint metadata caching that keeps Merkle recomputation off the live snapshot request path, independent stalled snapshot chunk retries, RocksDB read-only checkpoint serving and non-mutating checkpoint metadata verification, shielded warp-snapshot state, deterministic legacy testnet bootstrap-grant replay, signed validator bootstrap-recovery reclassification, stale-checkpoint repair protection after replay, recent canonical post-block effects recovery for stake-pool singleton convergence, shared BFT/block-receiver post-store hooks, four-validator public testnet validation, and the v0.5.93 stable validator base without replaying historical state incorrectly. `v0.5.187` also keeps the v0.5.151 snapshot transport hardening, drains stale pre-consensus BFT queues while joiners catch up, and keeps fresh-join initial sync on the batched block-range requester instead of broadcasting overlapping parent-gap repairs.
...
Examples:
- `https://github.com/lobstercove/lichen/releases/download/v0.5.187/lichen-validator-linux-x86_64.tar.gz`
- `https://github.com/lobstercove/lichen/releases/download/v0.5.187/lichen-validator-darwin-aarch64.tar.gz`
- `https://github.com/lobstercove/lichen/releases/download/v0.5.187/lichen-validator-windows-x86_64.tar.gz`
