...
**Current release candidate:** `v0.5.189` with `v0.5.188` retained as the signed rollback point. It preserves backed wallet and explorer account activity through clean validator rejoin, resumed sync, and state-repair snapshots; rebuilds account transaction counters from existing backed rows before new block deltas; and keeps full/fresh snapshot imports from merging stale target history while repair snapshots can merge verified public history indexes. It also keeps the `v0.5.188` four-validator proposer-rotation fix, exact legacy testnet MossStake replay boundaries, canonical block-archive checkpoint snapshots with sorted commit certificates, canonical ledger snapshot exports derived from slot mappings instead of raw noncanonical hot RocksDB history, authenticated PQ checkpoint sources, staged checkpoint snapshot imports, shielded warp-snapshot state, first-class BTC/wBTC support, deterministic legacy bootstrap-grant replay, signed bootstrap-recovery reclassification, recent canonical post-block effects recovery for stake-pool singleton convergence, and the v0.5.93 stable validator base without replaying historical state incorrectly.
...
Examples:
- `https://github.com/lobstercove/lichen/releases/download/v0.5.189/lichen-validator-linux-x86_64.tar.gz`
- `https://github.com/lobstercove/lichen/releases/download/v0.5.189/lichen-validator-darwin-aarch64.tar.gz`
- `https://github.com/lobstercove/lichen/releases/download/v0.5.189/lichen-validator-windows-x86_64.tar.gz`
