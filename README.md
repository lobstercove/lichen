...
**Current release candidate:** `v0.5.219`. Keep `v0.5.215` as the signed rollback point until a newer signed rollback anchor is explicitly recorded. This candidate preserves the `v0.5.215` exchange-readiness surface, removes unsafe fast nil-voting for missed proposers, hardens validator restart/rejoin recovery, and restores faucet-funded exchange simulation by signing faucet transfers with the configured faucet keypair instead of requiring validators to hold treasury signing material. Cold storage is attached before startup classifies local state, stale bootstrap markers cannot scrub validators with real progress, live snapshot rollback does not require hot archive rows that have already migrated to cold storage, and fresh archive imports are covered for mixed hot/cold history with matching state-root and public-history digests.
...
Examples:
- `https://github.com/lobstercove/lichen/releases/download/v0.5.219/lichen-validator-linux-x86_64.tar.gz`
- `https://github.com/lobstercove/lichen/releases/download/v0.5.219/lichen-validator-darwin-aarch64.tar.gz`
- `https://github.com/lobstercove/lichen/releases/download/v0.5.219/lichen-validator-windows-x86_64.tar.gz`
