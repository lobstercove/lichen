...
**Current release:** signed `v0.5.124` with first-class BTC/wBTC support, slot-only MossStake state, shielded warp-snapshot state, explicit shielded state-root activation support, deterministic legacy testnet bootstrap-grant replay, stale-checkpoint repair protection after replay, initial sync descendant catch-up, four-validator public testnet validation, and the v0.5.93 stable validator base without replaying historical state.
