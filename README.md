...
**Current release candidate:** `v0.5.191` with `v0.5.190` retained as the signed rollback point. It keeps archive-backed public RPC mandatory outside dev mode, uses consensus-state-only repair snapshots for fresh/resumed validator repair, and restores real public account/block/transaction history through the guarded read-only-source public-history merge path without replacing balances, validator state, contract storage, tip cursors, or other consensus data. It also preserves backed wallet and explorer account activity through clean validator rejoin, rebuilds account transaction counters from existing backed rows before new block deltas, keeps full/fresh archive snapshot imports from merging stale target history, keeps the four-validator proposer-rotation fix, exact legacy testnet MossStake replay boundaries, canonical block-archive checkpoint snapshots with sorted commit certificates, canonical ledger snapshot exports derived from slot mappings instead of raw noncanonical hot RocksDB history, authenticated PQ checkpoint sources, staged checkpoint snapshot imports, shielded warp-snapshot state, first-class BTC/wBTC support, deterministic legacy bootstrap-grant replay, signed bootstrap-recovery reclassification, recent canonical post-block effects recovery for stake-pool singleton convergence, and the v0.5.93 stable validator base without replaying historical state incorrectly.
...
Examples:
- `https://github.com/lobstercove/lichen/releases/download/v0.5.191/lichen-validator-linux-x86_64.tar.gz`
- `https://github.com/lobstercove/lichen/releases/download/v0.5.191/lichen-validator-darwin-aarch64.tar.gz`
- `https://github.com/lobstercove/lichen/releases/download/v0.5.191/lichen-validator-windows-x86_64.tar.gz`
