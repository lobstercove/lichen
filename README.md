...
**Current release:** signed `v0.5.131` with first-class BTC/wBTC support, slot-only MossStake state, full hot column-family checkpoint snapshots, live-safe cached-root checkpoint metadata, shielded warp-snapshot state, deterministic legacy testnet bootstrap-grant replay, signed validator bootstrap-recovery reclassification, stale-checkpoint repair protection after replay, recent canonical post-block effects recovery for stake-pool singleton convergence, initial sync descendant catch-up, four-validator public testnet validation, and the v0.5.93 stable validator base without replaying historical state.
