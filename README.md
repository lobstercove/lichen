...
**Current release:** signed `v0.5.141` with first-class BTC/wBTC support, slot-only MossStake state, full hot column-family checkpoint snapshots, streamed checkpoint snapshot imports through a verified staging DB, partial-archive checkpoint source rejection, pinned checkpoint repair sources, bounded checkpoint-metadata probes, background verified checkpoint metadata caching that keeps Merkle recomputation off the live snapshot request path, independent stalled snapshot chunk retries, RocksDB read-only checkpoint serving and non-mutating checkpoint metadata verification, shielded warp-snapshot state, deterministic legacy testnet bootstrap-grant replay, signed validator bootstrap-recovery reclassification, stale-checkpoint repair protection after replay, recent canonical post-block effects recovery for stake-pool singleton convergence, shared BFT/block-receiver post-store hooks, initial sync descendant catch-up, four-validator public testnet validation, and the v0.5.93 stable validator base without replaying historical state.
