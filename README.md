...
**Current release:** signed `v0.5.121` with first-class BTC/wBTC support, slot-only MossStake state, deterministic legacy testnet bootstrap-grant replay, root-verified checkpoint sync for same-slot state repair, live BFT restart catch-up recovery, four-validator public testnet validation, and the v0.5.93 stable validator base without replaying historical state.
