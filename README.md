...
**Current release:** signed `v0.5.93` with MossStake tier-weighted redemption, wall-clock staking locks/cooldowns with legacy testnet timestamp backfill, position-bound boosted lock tiers, pro-rata Flexible stLICN backing transfers, wallet/extension accrued-reward displays, wallet/DEX canonical signing fixes, bridge active-route deposit reservations, QR-backed bridge deposit displays, trusted atomic sparse dirty markers and clean sparse-startup rebuild skipping, persisted DEX orderbook/trade read indexes and startup backfill, slot-aware RPC read caching, lightweight WebSocket block/transaction fanout summaries, conditional stake-pool reloads after block apply, rebuildable DEX pair/taker trade-history read indexes, startup sparse-commitment repair before tip anchoring, guarded current-tip `post_state_v1` account-proof sidecar repair, canonical-apply-barrier account-proof reads, post-state account-proof anchors for current testnet proofs, sparse-by-default genesis for reset/local/mainnet launches, sparse state-commitment activation reporting, sparse account-proof support, coordinated testnet activation tooling, DEX input/default/voting-period fixes, observed-vs-target cadence reporting, live BFT catch-up message draining, release-wide binary hash verification for custody/faucet, DEX Pages signed-metadata asset gates, cached Merkle subroots for faster proposal roots, stale BFT proposal guards under the canonical apply barrier, dirty-marker-safe composite state-root caching, signed shielded private transfers, encrypted output-note recovery, private-transfer input/self-send gates, wallet and extension WebSocket reconnect hardening, DEX limit-first margin entry, full multi-pair DEX E2E coverage, prediction trader-position reads for Base58 wallets, stale CLOB level repair, wallet account-state isolation, local Neo X custody validation, full Neo X GAS/NEO custody routing, wNEO/wGAS contracts, Neo GAS rewards vault, reserve/liability proof services, agent/compute policy gates, wallet/DEX/RPC/SDK surfaces, DEX chart precision for Neo pairs, independent validator joiner P2P admission, stale-restart recovery, native shield proof witness preservation, durable encrypted shielded-note recovery, indexed shielded privacy reads, and fail-closed public activation gates built on the stable `v0.5.37` base.
...
Examples:
- `https://github.com/lobstercove/lichen/releases/download/v0.5.93/lichen-validator-linux-x86_64.tar.gz`
- `https://github.com/lobstercove/lichen/releases/download/v0.5.93/lichen-validator-darwin-aarch64.tar.gz`
- `https://github.com/lobstercove/lichen/releases/download/v0.5.93/lichen-validator-windows-x86_64.tar.gz`
