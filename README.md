...
**Current testnet release and restart-safe anchor:** `v0.5.228`. Signed `v0.5.225` is preserved as the pre-change artifact but is not restart-safe on the legacy testnet. Signed `v0.5.227` fixed the restart recovery boundary and is safe for canonical nodes, but its legacy one-node repair command fails closed because it conflates total and spendable balances; do not execute that command. v0.5.228 corrects the source-bound repair and retains the emergency bridge: complete hot/cold archive reads, a 5 GiB floor only for the exact testnet selector, and a 50,000-slot default hot/cold boundary while Archive V2 is implemented. The existing testnet has an explicit legacy-history waiver; fresh networks and mainnet fail closed on incomplete genesis-to-tip history.
...
Examples:
- `https://github.com/lobstercove/lichen/releases/download/v0.5.228/lichen-validator-linux-x86_64.tar.gz`
- `https://github.com/lobstercove/lichen/releases/download/v0.5.228/lichen-validator-darwin-aarch64.tar.gz`
- `https://github.com/lobstercove/lichen/releases/download/v0.5.228/lichen-validator-windows-x86_64.tar.gz`
