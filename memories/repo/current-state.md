...
- The live 3-VPS testnet fleet must be reset again from `v0.5.13` before production promotion; the failed `v0.5.10` run, the restart-noisy `v0.5.11` run, and the marker-noisy `v0.5.12` run should be treated as superseded.
...
- The 2026-04-23 production-pass handover records the `v0.5.7` hardening release contents; release docs now target `v0.5.13`, while the live fleet remains date-aware until the replacement clean-slate redeploy is verified.
