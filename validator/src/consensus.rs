...
mod tests {
    ...
    fn assert_equal_stake_degraded_set_progresses(total: usize, offline: usize) {
        let (validators, vs, sp) = make_test_env(total);
        let offline_set: std::collections::HashSet<Pubkey> = validators
            .iter()
            .take(offline)
            .map(|(_, pubkey)| *pubkey)
            .collect();
        let online_voters: Vec<Pubkey> = validators
            .iter()
            .map(|(_, pubkey)| *pubkey)
            .filter(|pubkey| !offline_set.contains(pubkey))
            .collect();
        let (engine_keypair, engine_pubkey) = make_validator(1);
        let engine = ConsensusEngine::new(engine_keypair, engine_pubkey);
        assert!(
            engine.has_supermajority_voters(&online_voters, &vs, &sp),
            "{}/{} equal-stake online validators must retain BFT quorum",
            online_voters.len(),
            total
        );

        for height in 1..=total as u64 * 4 {
            let parent_hash = Hash::hash(&height.to_be_bytes());
            let first_online_round = (0..total as u32)
                .find(|round| {
                    ...
                    vs.select_leader_weighted(leader_slot, &sp, &parent_hash.0, MIN_VALIDATOR_STAKE)
                        .is_some_and(|leader| !offline_set.contains(&leader))
                })
                .expect("one complete rotation must contain an online proposer");
            assert!(
                first_online_round <= offline as u32,
                "height {height} needed round {first_online_round} with {offline}/{total} offline"
            );
        }
    }

    #[test]
    fn test_four_validator_set_progresses_with_one_offline() {
        assert_equal_stake_degraded_set_progresses(4, 1);
    }

    #[test]
    fn test_ten_validator_set_progresses_with_two_offline() {
        assert_equal_stake_degraded_set_progresses(10, 2);
    }
