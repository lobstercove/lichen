...
fn rpc_tip_from_json_response(body: &serde_json::Value) -> Option<u64> {
    body["result"]
        .as_u64()
        .or_else(|| body["result"]["slot"].as_u64())
}

async fn fetch_rpc_method_slot(
    ...
    rpc_tip_from_json_response(&body)
}

async fn fetch_rpc_slot(http_client: &reqwest::Client, rpc_url: &str) -> Option<u64> {
    match fetch_rpc_method_slot(http_client, rpc_url, "getSlot").await {
        Some(slot) => Some(slot),
        None => fetch_rpc_method_slot(http_client, rpc_url, "getHealth").await,
    }
}
...
mod tests {
    ...
    #[test]
    fn bootstrap_tip_probe_reads_health_slot_when_get_slot_is_fail_closed() {
        let slot_body = serde_json::json!({
            "jsonrpc": "2.0",
            "id": 1,
            "result": 1173885u64
        });
        assert_eq!(rpc_tip_from_json_response(&slot_body), Some(1173885));
        ...
        let health_body = serde_json::json!({
            ...
            "result": {
                ...
                "reason": "stale_tip",
                "slot": 1173885u64,
                "block_age_secs": 866u64
            }
        });
        assert_eq!(rpc_tip_from_json_response(&health_body), Some(1173885));

        let fail_closed_get_slot = serde_json::json!({
            "jsonrpc": "2.0",
            "id": 1,
            "error": {
                "code": -32050,
                "message": "RPC node is not ready: local tip is stale at slot 1173885 (age 866s)"
            }
        });
        assert_eq!(rpc_tip_from_json_response(&fail_closed_get_slot), None);
    }
